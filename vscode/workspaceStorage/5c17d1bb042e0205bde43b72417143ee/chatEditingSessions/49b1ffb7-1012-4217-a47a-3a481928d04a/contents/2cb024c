...
export async function getRecommendedProducts() {
  try {
    ...
    // Extract unique category IDs from customer's purchase history
    const purchasedCategoryIds = purchasedProducts
      .flatMap((item) => item.product.categories)
      .map((cat) => cat.categoryId);

    const uniqueCategoryIds = [...new Set(purchasedCategoryIds)];

    // If no purchase history, return popular products instead
