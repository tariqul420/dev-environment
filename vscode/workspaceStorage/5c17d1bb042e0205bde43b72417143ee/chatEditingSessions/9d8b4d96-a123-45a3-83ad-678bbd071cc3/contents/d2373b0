    ...
    >
      ...
      {!isBusy && (
        <Button
          ...
          onClick={() =>
            logger.info(`🎯 Drag handle clicked for file: ${file.id}`)
          }
        >
...
export default function ImageUploaderField({
  ...
  const handleDragStart = useCallback((event: DragStartEvent) => {
    logger.info(`🚀 Drag started for: ${event.active.id}`);
  }, []);
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (!over || active.id === over.id) {
        logger.info("❌ Drag cancelled or same position");
        return;
      }
      ...
      if (activeIndex === -1 || overIndex === -1) {
        logger.info("❌ Could not find file indices");
        return;
      }

      logger.info(
        `🔄 Drag end: moving item from position ${activeIndex} to position ${overIndex}`,
      );
