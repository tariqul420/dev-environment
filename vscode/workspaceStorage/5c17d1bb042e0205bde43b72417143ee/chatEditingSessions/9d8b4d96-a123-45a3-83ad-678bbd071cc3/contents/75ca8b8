...
export default function ImageUploaderField({
  ...
  }, [files, clearErrors, getList, removeFile, setList]);
    ...
    (image: SelectableImage) => {
      ...
      if (multiple) {
        if (currentUrls.includes(image.url)) {
          // Deselect - remove from list and clean up references
          const newUrls = currentUrls.filter((url) => url !== image.url);
          setList(newUrls);

          // Find and remove the file from useFileUpload
          const fileToRemove = files.find((f) => {
            const fileUrl = f.preview || idToUrl.current[f.id as string];
            return fileUrl === image.url;
          });
          
          if (fileToRemove) {
            removeFile(fileToRemove.id as string);
          }

          // Remove from idToUrl mapping
          Object.keys(idToUrl.current).forEach((id) => {
            if (idToUrl.current[id] === image.url) {
              delete idToUrl.current[id];
            }
          });
        } else {
          // Select - add to list
          const newUrls = [...currentUrls, image.url];
          setList(newUrls);

          // Create a virtual file for the selected image and add it to useFileUpload
          const selectedImageId = `selected-${image.id}-${Date.now()}`;
          const virtualFile: FileWithPreview = {
            id: selectedImageId,
            preview: image.url,
            file: null, // No actual file, just the URL
          };

          // Add the virtual file to the files array by creating an initial file
          const newInitialFile: InitialFileMeta = {
            id: selectedImageId,
            name: image.alt || `selected-image-${image.id}`,
            size: 0,
            type: "image/jpeg",
            url: image.url,
          };

          // Add to idToUrl mapping
          idToUrl.current[selectedImageId] = image.url;

          // Force re-render by updating the files through initialFiles
          setTimeout(() => {
            window.location.reload = window.location.reload; // Trigger a state update
          }, 0);
        }
      } else {
        setList([image.url]);
        ...
        files.forEach((f) => removeFile(f.id as string));

        // Clear existing mappings and add new one
        idToUrl.current = {};
        const selectedImageId = `selected-${image.id}-${Date.now()}`;
        idToUrl.current[selectedImageId] = image.url;
      }
    },
    [getList, setList, multiple, files, removeFile],
  );
