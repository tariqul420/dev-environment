  ...
  try {
    ...
    const grouped = await prisma.$queryRaw<
      Array<{
        productId: string;
        qty_sum: bigint;
        total_sum: { toNumber(): number };
      }>
    >`
      SELECT 
        oi."productId",
        CAST(SUM(oi."qty") AS BIGINT) as qty_sum,
    ...
    const transformedGrouped = grouped.map((item) => ({
      productId: item.productId,
      _sum: {
        qty: Number(item.qty_sum),
        total: item.total_sum.toNumber(),
      },
    }));

    const productIds = transformedGrouped.map((g) => g.productId);
    ...
    const trendingProducts: TrendingProduct[] = transformedGrouped
      .map((g) => {
      ...
      .filter((item) => item !== null) as TrendingProduct[];
