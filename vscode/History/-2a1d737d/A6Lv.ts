...
type IUserExtend = IUser & { _id: Types.ObjectId }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    logger.error("Error: Could not verify webhook", { error: err });
  ...
  if (eventType === "user.created") {
    ...
    try {
      const newUser = await createUser(user as IUserExtend);
      ...
      if (newUser) {
        try {
          ...
          logger.error("Error updating Clerk user metadata", { error });
        }
      }
  ...
  if (eventType === "user.updated") {
    ...
    try {
      const updatedUser = await updateUser(id, IUserExtend);
