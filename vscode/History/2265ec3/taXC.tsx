...
export default function DiscountFinderClient() {
  ...
  const [original, setOriginal] = useState<string>(qs("op", "1200") || "1200");
  const [discountPct, setDiscountPct] = useState<string>(qs("dp", "15") || "15");
  const [extraPct, setExtraPct] = useState<string>(qs("xp", "0") || "0");
  const [fixedOff, setFixedOff] = useState<string>(qs("fx", "0") || "0");
  const [taxPct, setTaxPct] = useState<string>(qs("tx", "0") || "0");
  const [qty, setQty] = useState<string>(qs("q", "1") || "1");
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {copied === "summary" ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />} Copy
          Summary
        </Button>
        <Button
          variant="outline"
