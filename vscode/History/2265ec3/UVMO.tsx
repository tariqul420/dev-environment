...
export default function DiscountFinderClient() {
  ...
  const {
    ...
    // apply sequential discounts: original -> %disc -> extra% -> -fixed -> +tax
    ...
    if (o > 0 && 1 - xp / 100 > 0 && tf > 0) {
      const rhs = tf / (1 + tx / 100) + fx; // target before tax, before fixed
      const denom = o * (1 - xp / 100);
      const oneMinusDp = denom > 0 ? rhs / denom : 0; // 1 - required_dp
  ...
  return (
    <>
      <ToolPageHeader
        ...
        description="Before/after price, savings, and effective discount. Reverse mode can tell you the % needed to hit a target final price."
        actions={
          <>
            ...
            />
            <ActionButton icon={Calculator} label="Save Result" onClick={saveHistory} />
          </>
        }
      />
