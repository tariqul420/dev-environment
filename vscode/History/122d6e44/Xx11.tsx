...
function toSentenceCase(s: string) {
  return s
    .toLowerCase()
    .replace(/(^\s*[a-z\p{Ll}]|[.!?]\s*[a-z\p{Ll}])/gu, (m) => m.toUpperCase());
}
...
function normalizeSmartChars(
  ...
  opts: Pick<
    CleanOptions,
    "normalizeQuotes" | "normalizeDashes" | "replaceEllipsis"
  >,
) {
  let r = s;
...
export default function ClipboardCleanerClient() {
  ...
  const pushHistory = (src: string, out: string) => {
    if (!out.trim()) return;
    setHistory((h) =>
      [{ id: uid("h"), ts: Date.now(), src, out }, ...h].slice(0, 20),
    );
  };
            ...
            <TextareaField
              ...
              onPaste={(e) => {
                ...
                if (pasted) {
                  ...
                  setRaw(
                    (prev) =>
                      prev.slice(0, selStart) + out + prev.slice(selEnd),
                  );
                }
              }}
