// app/tools/(tools)/url/qr/page.tsx
'use client';
...
export default function AdvancedQRPage() {
  ...
  const effectiveFg = useMemo(() => {
    if (!autoFg) return fg;
    // sample the middle of the background
    const sampleBg = useGradient ? mixHex(gradFrom, gradTo, 0.5) : bg;
    // prefer dark ink if contrast is OK, otherwise white
    const dark = '#111827';
    const light = '#ffffff';
    return contrastRatio(dark, sampleBg) >= 3 ? dark : light;
  }, [autoFg, useGradient, gradFrom, gradTo, bg, fg]);
  ...
  const [rounded, setRounded] = useState(24); // container rounding
  const [label, setLabel] = useState('');
  const [labelPos, setLabelPos] = useState<'top' | 'bottom'>('bottom');

  // Background gradient behind (QR bg transparent when gradient)
  ...
  // Gradient INSIDE QR modules (advanced)
  ...
  const [logoBorder, setLogoBorder] = useState(0); // px stroke around logo BG
  ...
  // ---------- inject gradient into QR modules if enabled ----------
  // replace your gradient injection useEffect with this
  useEffect(() => {
    ...
    // make sure modules are filled squares (not stroked outlines)
    paths.forEach((p) => {
    ...
    svg.querySelector('#qr-fg-grad')?.remove();

    if (useQrGradient) {
      // build gradient
      let grad: SVGLinearGradientElement | SVGRadialGradientElement;
      if (gradDir === 'radial') {
        grad = document.createElementNS('http://www.w3.org/2000/svg', 'radialGradient');
        grad.setAttribute('id', 'qr-fg-grad');
        grad.setAttribute('cx', '50%');
        grad.setAttribute('cy', '50%');
        grad.setAttribute('r', '75%');
      } else {
        const linear = document.createElementNS('http://www.w3.org/2000/svg', 'linearGradient');
        linear.setAttribute('id', 'qr-fg-grad');
        linear.setAttribute('x1', '0%');
        linear.setAttribute('y1', '0%');
        linear.setAttribute('x2', gradDir === 'to-r' ? '100%' : '0%');
        linear.setAttribute('y2', gradDir === 'to-b' ? '100%' : '0%');
        grad = linear;
      }

      const stop1 = document.createElementNS('http://www.w3.org/2000/svg', 'stop');
      stop1.setAttribute('offset', '0%');
      stop1.setAttribute('stop-color', gradFrom);
      const stop2 = document.createElementNS('http://www.w3.org/2000/svg', 'stop');
      stop2.setAttribute('offset', '100%');
      stop2.setAttribute('stop-color', gradTo);
      grad.appendChild(stop1);
      grad.appendChild(stop2);
      defs!.appendChild(grad);

      // ✅ apply gradient fill to modules
      paths.forEach((p) => p.setAttribute('fill', 'url(#qr-fg-grad)'));
    } else {
      // ✅ force solid fill
      paths.forEach((p) => p.setAttribute('fill', fg));
    }
  }, [useQrGradient, gradDir, gradFrom, gradTo, fg, value, size, level, margin]);
  ...
  const downloadSVG = () => {
    ...
    // If gradient is off, make sure path fill = fg
    if (!useQrGradient) {
      cloned.querySelectorAll('path').forEach((p) => p.setAttribute('fill', fg));
    }
    ...
    img.onload = () => {
      ...
      if (useGradient) {
        if (gradDir === 'radial') {
          ...
          // @ts-expect-error canvas typings
          ctx.fillStyle = g;
        } else {
      ...
      if (label) {
        ...
        const textY = labelPos === 'top' ? clamp(pad / 2, 0, canvas.height) : clamp(canvas.height - pad / 2, 0, canvas.height);
      ...
      if (hasLogo && (logoDataUrl || logoUrl)) drawLogo();
      else finish();
    };
  ...
  const share = async () => {
    ...
    try {
      await (navigator as any).share({ title: 'QR code', text: value, url: value.startsWith('http') ? value : undefined });
    } catch {}
  };

  // ---------- basic guidance ----------
  const warnECC = hasLogo && level !== 'H' && logoPct >= 18; // nudge: big logo prefers ECC H
  const contrast = useGradient ? Math.max(contrastRatio('#000000', gradFrom), contrastRatio('#000000', gradTo)) : contrastRatio(fg, bg);
  const okContrast = contrast >= 2.5; // QR scanners are forgiving; 2.5+ is usually fine

  return (
    <div className="container mx-auto max-w-6xl px-4 py-10">
      <SectionHeader title="QR Code (Advanced)" desc="Export-ready QR with gradients, label, logo overlay, builders (URL, Wi-Fi, vCard) and more." />
          ...
          <div className="grid gap-6">
            ...
            <GlassCard className="p-4">
              ...
              <div className="mt-3 grid gap-3">
                ...
                {contentType === 'text' && <Input value={textValue} onChange={(e) => setTextValue(e.target.value)} placeholder="Freeform text…" className="bg-background/60 backdrop-blur" />}
            ...
            <GlassCard className="p-4">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <span className="ml-4">Inked gradient (QR)</span>
                  <Switch checked={useQrGradient} onCheckedChange={setUseQrGradient} />
                </div>
              </div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                {okContrast ? (
                  ...
                  <Badge className="gap-1" variant="destructive">
                    <AlertTriangle className="h-3.5 w-3.5" /> Low contrast
                  </Badge>
                )}
                {warnECC && (
                  <Badge className="gap-1" variant="outline">
                    <AlertTriangle className="h-3.5 w-3.5" /> Large logo detected — ECC H recommended
                  </Badge>
                )}
              </div>
            </GlassCard>
          ...
          <div className="grid gap-4">
            <GlassCard className="p-4">
              ...
              <div
                ...
                <div className="relative" ref={svgWrapperRef} style={{ width: size, height: size, lineHeight: 0 }}>
                  <QRCode value={value || ' '} size={size} level={level} fgColor={useQrGradient ? '#000000' : fg} bgColor={useGradient ? 'transparent' : bg} className="block" />
                  {hasLogo && (
                    <div
