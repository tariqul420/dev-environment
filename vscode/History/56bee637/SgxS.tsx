...
type Props = {
  data: Point[];
  height?: number;
  /** Line / area color (defaults to shadcn primary) */
  color?: string;
};

export default function ClicksByDayChart({ data, height = 220, color = '#fff5' }: Props) {
  const id = React.useId();
  const gradientId = `fillClicks-${id}`;

  // higher contrast tokens for dark cards
  const grid = 'hsl(var(--border) / 0.5)'; // grid lines
  const tick = 'hsl(var(--foreground) / 0.8)'; // axis labels
  const tooltipBg = 'hsl(var(--popover))';
  const tooltipFg = 'hsl(var(--popover-foreground))';
  const tooltipBorder = 'hsl(var(--border))';
  const dotFill = 'hsl(var(--card))'; // dot center (visible on dark)
    ...
    <div style={{ height }} className="w-full">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
          ...
          {/* Built-in tooltip, styled via shadcn variables */}
          <Tooltip
            cursor={{ stroke: grid }}
