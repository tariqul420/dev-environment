...
import { Calculator, Delete, Equal, Eraser, FunctionSquare, Parentheses, Percent, Pi, SquareRoot, Superscript } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-10">
      ...
      <MotionGlassCard className="p-4">
        {/* header strip */}
        <GlassCard className="mb-3 flex items-center justify-between rounded-xl p-3 text-xs text-muted-foreground">
          <div>Functions: sin, cos, tan, log₁₀, ln, √, ^, π, e</div>
          {lastAns && <div className="font-medium">ANS: {lastAns}</div>}
        </GlassCard>
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <div className="col-span-4 flex gap-2 overflow-x-auto lg:col-span-4 lg:grid lg:grid-cols-6 lg:gap-3">
            {FNS.map((f) => (
              <CalcButton key={f.label} onClick={() => push(f.ins)} variantIntent="accent" className="min-w-[80px] justify-center">
                {f.label === '√' ? <SquareRoot className="h-4 w-4" /> : f.label}
              </CalcButton>
            ))}
          </div>
