...
export default function ScientificCalculatorPage() {
  ...
  const copyExpr = React.useCallback(async () => {
    try {
  ...
  const copyAns = React.useCallback(async () => {
    try {
      const v = ans || expr;
      if (!v) return;
  ...
  const pasteClipboard = React.useCallback(async () => {
    try {
      ...
      if (!text) return;
      // Allow digits, ops, spaces, letters for functions, pi symbol, caret, parentheses, percent
      const sanitized = text.replace(/[^0-9.+\-*/()% ()A-Za-zπ^√]/g, "");
  ...
  return (
    <div className="container mx-auto max-w-4xl px-4 py-8 space-y-4">
      ...
      <ToolPageHeader
        ...
        description="Trig, logs, powers, constants, and ANS. Live preview updates as you type."
        actions={
          <>
            <ResetButton onClick={resetAll} />
            <ActionButton icon={Eraser} variant="outline" label="Clear" onClick={clear} />
            <ActionButton icon={Delete} variant="outline" label="Back" onClick={back} />
            <CopyButton label="Copy Expr" getText={() => expr || "0"} />
            <CopyButton label="Copy Result" getText={() => (ans || expr || "0").toString()} />
            <ActionButton
              icon={Calculator}
              variant="default"
              label="Equals"
      ...
      <div className="mb-1 flex flex-wrap gap-2">
        <Link href="/tools/calc/standard" className="contents">
          <CalcButton variantIntent="ghost" className="px-3">
            <Calculator className="mr-2 h-4 w-4" />
            Standard
          </CalcButton>
        </Link>
        <Link href="/tools/calc/scientific" className="contents">
          <CalcButton variantIntent="primary" className="px-3">
            <FunctionSquare className="mr-2 h-4 w-4" />
            Scientific
          </CalcButton>
        </Link>
        <Link href="/tools/calc/percentage" className="contents">
          <CalcButton variantIntent="ghost" className="px-3">
      ...
      <MotionGlassCard className="p-4">
        {/* 4-column grid keeps Display spanning nicely */}
        <div className="grid grid-cols-4 gap-3">
          {/* Display */}
          <Display value={expr || "0"} hint={previewHint} />

          {/* Function bank — horizontal scroll on mobile, grid on larger screens */}
          <div
            className={cn(
          ...
          >
            {FNS.map((f) => (
              ...
              >
                {/* Avoid fragile icon imports; render label directly */}
                {f.label}
              </CalcButton>
            ))}
          </div>
