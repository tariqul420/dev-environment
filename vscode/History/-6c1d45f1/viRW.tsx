...
export default function TimestampConverterClient() {
  ...
  const runToDate = useCallback(
    (nowSec: number) => run("toDate", String(nowSec), unit, tz, customTz, dateText),
    [run, unit, tz, customTz, dateText],
  );
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (dir === "toDate") runToDate(nowSec);
    }, 1000);
    return () => clearInterval(id);
  }, [autoTick, dir, runToDate]); // ✓ clean deps
    ...
    <>
      ...
      <GlassCard>
        <CardContent className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
          {/* Left: Inputs */}
          <div className="rounded-xl border p-4 space-y-4 h">
