...
export default function NotificationsDropdown({
      ...
      try {
        ...
        if (!ignore) {
          ...
          const filtered = data.filter((n) => {
            ...
            // customer শুধু order/support/system (stock hide)
            return n.type !== "stock";
          });
          setItems(filtered);
        }
  ...
  // Optional: Socket.IO realtime (enable if you use it)
  // React.useEffect(() => {
  //   let socket: any;
  //   (async () => {
  ...
  //       // নতুন অর্ডার নোটিফিকেশন পুশ
  //       setItems(prev => [
  //         {
  //           _id: crypto.randomUUID(),
  //           type: "order",
  //           title: `New order #${payload?.orderId ?? ""}`,
  //           message: payload?.name ? `${payload.name} just ordered` : "A new order arrived",
  //           createdAt: new Date().toISOString(),
  //           read: false,
  //           href: `/dashboard/orders/${payload?._id ?? ""}`,
  //           meta: payload,
  //         },
  //         ...prev,
  //       ]);
