...
function hexToRgb(hex: string): RGB | null {
  ...
  if (h.length === 3)
    h = h
      ...
      .map((c) => c + c)
      .join("");
  if (h.length === 8) h = h.slice(2); // ignore alpha
...
export default function ColorConverterClient() {
  ...
  // save on hex change — functional update avoids recent in deps (fixes exhaustive-deps warning)
  React.useEffect(() => {
    const id = setTimeout(() => {
      try {
        setRecent((prev) => {
        ...
        <GlassCard>
          <div className="grid gap-4 p-4">
            ...
            <ColorField
              ...
              value={hex}
              onChange={(v) => {
                const hexVal = typeof v === "string" ? v : (v?.hex ?? "");
                const rgbNew = hexToRgb(hexVal);
                  ...
                  onChange={(e) => {
                    ...
                    if (parsed) {
                      ...
                      if (v.toLowerCase().startsWith("rgba")) {
                        const m = RGB_CSV_RX.exec(v);
                        if (m && m[4] != null) setAlpha(clamp(parseFloat(m[4]!), 0, 1));
                      }
                    }
                  }}
                  inputClassName="font-mono"
            ...
            <div className="flex items-end gap-2">
              <InputField
                ...
                placeholder="#09f, rgb(...), hsl(...), hsv(...), #AARRGGBB"
                value={""}
                onChange={() => {}}
                readOnly
              />
