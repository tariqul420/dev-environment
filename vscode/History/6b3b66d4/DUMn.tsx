...
function useLeafletDefaultIcon() {
  React.useEffect(() => {
    ...
    // Dynamic import so SSR never touches Leaflet (no "window is not defined")
    import("leaflet").then((mod) => {
      const L = (mod as any).default ?? mod; // <-- handles both ESM/CJS shapes
      if (cancelled) return;

      // @ts-expect-error: patch private prop at runtime
