...
import { Check, Copy, Download, Hash, RefreshCw, RotateCcw, Settings2 } from 'lucide-react';
...
function makeOrderId(prefix = 'ORD', dateFmt: 'epoch' | 'YMDHM' | 'YMD' = 'YMDHM', suffix = '') {
  ...
  const base = dateFmt === 'epoch' ? String(d.getTime()) : dateFmt === 'YMD' ? `${y}${m}${day}` : `${y}${m}${day}-${hh}${mm}`;
...
export default function IdGeneratorPage() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('tools:idgen:state', JSON.stringify({ count, mode, uuidHyphens, nanoidLen, nanoidAlphabet, ensureUnique, ordPrefix, ordSuffix, ordFmt, shortPrefix }));
    } catch {}
  ...
  const exportTxt = () => download('ids.txt', items.join(''), 'text/plain');
  const exportCsv = () => {
    const rows = ['index,type,id', ...items.map((id, i) => `${i + 1},${mode},${id}`)];
    download('ids.csv', rows.join(''), 'text/csv');
  };
          ...
          <CardContent className="grid gap-4 lg:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                <Button variant={mode === 'uuid' ? 'default' : 'outline'} onClick={() => setMode('uuid')}>
                  UUID
                </Button>
                <Button variant={mode === 'uuid-nh' ? 'default' : 'outline'} onClick={() => setMode('uuid-nh')}>
                  UUID (no -)
                </Button>
                <Button variant={mode === 'ulid' ? 'default' : 'outline'} onClick={() => setMode('ulid')}>
                  ULID
                </Button>
                <Button variant={mode === 'nanoid' ? 'default' : 'outline'} onClick={() => setMode('nanoid')}>
                  NanoID
                </Button>
                <Button variant={mode === 'order' ? 'default' : 'outline'} onClick={() => setMode('order')}>
                  OrderID
                </Button>
                <Button variant={mode === 'short' ? 'default' : 'outline'} onClick={() => setMode('short')}>
                  ShortID
                </Button>
              </div>
            </div>
              ...
              {mode === 'uuid' || mode === 'uuid-nh' ? (
                <div className="text-sm">
                  <div className="flex items-center gap-2">
                    <Switch checked={uuidHyphens} onCheckedChange={setUuidHyphens} /> Keep hyphens
                  </div>
                </div>
              ) : null}
              {mode === 'nanoid' ? (
              ...
              {mode === 'order' ? (
                <div className="grid gap-2 text-sm">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <Button size="sm" variant={ordFmt === 'YMDHM' ? 'default' : 'outline'} onClick={() => setOrdFmt('YMDHM')}>
                      YYYYMMDD-HHMM
                    </Button>
                    <Button size="sm" variant={ordFmt === 'YMD' ? 'default' : 'outline'} onClick={() => setOrdFmt('YMD')}>
                      YYYYMMDD
                    </Button>
                    <Button size="sm" variant={ordFmt === 'epoch' ? 'default' : 'outline'} onClick={() => setOrdFmt('epoch')}>
                      Epoch
                    </Button>
                  </div>
                </div>
