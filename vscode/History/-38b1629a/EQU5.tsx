...
export default function CategoryForm({ onCategoryCreated }: CategoryFormProps) {
  ...
  return (
    <div className="flex items-center space-x-2">
      <div className="bg-light font-grotesk flex w-full items-center gap-2 overflow-hidden rounded-md dark:bg-transparent">
        <Input placeholder="Enter category name" onChange={(e) => setCategory(e.target.value)} value={category} />
      </div>
      <Button type="button" onClick={onSubmit} variant="outline" disabled={loading}>
        <IconPlus size={12} />
      </Button>
    </div>
  );
