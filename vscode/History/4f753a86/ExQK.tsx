...
import { Check, Copy, Download, Eraser, Highlighter, ImageOff, Info, Layers, Loader2, MagicWand, Palette, Pipette, RotateCcw, Upload } from 'lucide-react';
...
export default function BgRemovePage() {
  ...
  // Chroma
  const [keyHex, setKeyHex] = React.useState('#ffffff'); // bg color
  ...
  const onDrop = React.useCallback(async (files: File[]) => {
    ...
    // initialize canvases next tick
    requestAnimationFrame(() => initCanvases(url, meta.width, meta.height, canvasRef, overlayRef));
  }, []);
  ...
  async function run() {
    ...
    try {
      ...
      // Build mask
      let mask = new Uint8ClampedArray(base.width * base.height); // 0 keep, 255 remove
      if (mode === 'auto') {
        ...
        if (overlayRef.current) {
          mask = overlayEraseMask(overlayRef.current);
        }
      }
  ...
  React.useEffect(() => {
    ...
    function drawDot(p: { x: number; y: number }) {
      ...
      ctx.globalCompositeOperation = brushErase ? 'source-over' : 'destination-out'; // erase = paint white; restore = cut hole
      ctx.fillStyle = brushErase ? 'rgba(255,255,255,1)' : 'rgba(255,255,255,1)';
      ctx.beginPath();
        ...
        <GlassCard className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-6">
          <div>
            <h1 className="flex items-center gap-2 text-2xl font-semibold tracking-tight">
              <MagicWand className="h-6 w-6" /> Background Remover
            </h1>
...
function floodMask(base: ImageData, tol: number): Uint8ClampedArray {
  ...
  while (q.length) {
    ...
    // background if near any seed color
...
// Manual: interpret overlay white as "erase"
function overlayEraseMask(overlay: HTMLCanvasElement): Uint8ClampedArray {
  ...
  for (let i = 0, p = 0; i < data.length; i += 4, p++) {
    out[p] = data[i + 3] > 0 ? 255 : 0; // any painted pixel → remove
  }
  return out;
}
