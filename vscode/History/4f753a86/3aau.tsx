...
export default function BgRemovePage() {
  ...
  React.useEffect(() => {
    ...
    function drawDot(p: { x: number; y: number }) {
      ...
      if (!ctx) return;
      // erase = paint white (will be read as alpha>0 mask); restore = erase painted area
...
/** Initialize base and overlay canvases with the loaded image. */
async function initCanvases(srcUrl: string, w: number, h: number, baseRef: React.RefObject<HTMLCanvasElement>, overlayRef: React.RefObject<HTMLCanvasElement>) {
