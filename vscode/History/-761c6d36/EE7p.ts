...
export const getProjects = async ({ search, sort = 'default', page = 1, limit = 6, isFeatured = false }: ParamsPropsType) => {
  try {
    await dbConnect();
...
export const getProjectsForAdmin = async ({ page = 1, limit = 20, search = '' }: { page?: number; limit?: number; search?: string }) => {
  try {
    ...
    const [project, total] = await Promise.all([
      ProjectModel.find(query)
        .select('_id title isFeatured order liveUrl updatedAt slug')
        .skip((page - 1) * limit)
        .limit(limit)
        .sort({ updatedAt: -1 })
        .lean(),
