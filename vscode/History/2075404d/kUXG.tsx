...
function buildUnified(chunks: DiffChunk[], context = 3): string {
  ...
  const flush = () => {
    ...
    // count a/b lines in the pending hunk
  ...
  for (const chunk of chunks) {
    ...
    if (chunk.op === "equal") {
      if (!open) {
        // not inside a hunk, just advance counters
        aLine += left.length;
        bLine += left.length;
        continue;
      }
      // inside a hunk, include limited context then close
      ...
      flush();
      // after closing, advance counters for the skipped equal lines
...
function SplitView({
  ...
  // Render line-by-line for readability
