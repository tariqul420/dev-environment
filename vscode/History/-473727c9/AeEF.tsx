...
const DYNAMIC_KEYWORDS = buildDynamicKeywords(ToolsData as any);
...
const smartDescription = description || siteDescriptionFallback(ToolsData as any);
...
export default function ToolsLayout({ children }: { children: React.ReactNode }) {
  const navLd = {
    ...
    itemListElement: ToolsData.map((c: any, i: number) => ({
      "@type": "ListItem",
      position: i + 1,
      name: c.title,
      url: `${siteURL}${c.url}`,
