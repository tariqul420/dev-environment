...
export default function AgeCalculatorPage() {
  ...
  if (!birth || !results) return;
  const summary = `Age: ${results.ymd.years}y ${results.ymd.months}m ${results.ymd.days}d\nBorn: ${shortDate(birth)}\nTotal: ${results.total.days} days, ${results.total.hours} hours\nNext birthday: ${
    results.until.exact
  } (${results.until.days} days)`;
          ...
          {birth && results && (
            <>
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <Stat className="capitalize" label="Age" value={`${results.ymd.years}y ${results.ymd.months}m ${results.ymd.days}d`} hint={`Born ${shortDate(birth)}`} />
