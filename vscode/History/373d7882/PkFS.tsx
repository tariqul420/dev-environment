...
function cleanBaseUrl(url: string) {
  ...
  try {
    ...
    // Return without search unless opts.keepExisting says otherwise (handled later)
    ...
    try {
      ...
      return trimmed; // invalid; caller will validate
    }
  }
}
...
function parseExisting(url: string) {
  try {
    ...
    out.id = get('utm_id');
    // gather non-utm as custom disabled by default
    const custom: Pair[] = [];
    p.forEach((v, k) => {
...
function buildSingle(baseUrl: string, utm: UTMState, opts: OptionsState) {
  ...
  // ensure required ones exist? no - user decides; but we will not output incomplete warning here
...
export default function UTMBuilderPage() {
  ...
  const resultBatch = useMemo(() => {
    ...
    const lines = batchList
      ...
      .map((l) => l.trim())
      .filter(Boolean);
    return lines.map((b) => buildSingle(b, utm, opts));
  }, [batchList, utm, opts]);
  ...
  function pushHistory(singleOrBatch: 'single' | 'batch') {
    const item: HistoryItem = {
      ts: Date.now(),
      base: baseUrl,
      result: singleOrBatch === 'single' ? result : resultBatch,
    };
    try {
  ...
  function exportBatchCSV() {
    ...
    const rows: string[][] = [['Base URL', 'Result URL'], ...resultBatch.map((r, i) => [batchList.split('\n')[i]?.trim() || '', r])];
    csvDownload('utm-batch.csv', rows);
  }

  const canBuildSingle = baseUrl && isValidUrl(baseUrl) && !opts.batchMode;
  const canBuildBatch = opts.batchMode && batchList.trim().length > 0;
          ...
          {opts.batchMode && (
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder={`https://example.com\nhttps://naturalsefaa.com/blog/article`}
                value={batchList}
        ...
        <CardContent className="space-y-4">
          {!opts.batchMode && (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Button disabled={!result || requiredMissing.length > 0} onClick={() => pushHistory('single')} variant="outline" className="gap-2">
          ...
          {opts.batchMode && (
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline" className="gap-2" disabled={!resultBatch.length} onClick={() => copy(resultBatch.join('\n'))}>
                  <Copy className="h-4 w-4" /> Copy All
                </Button>
                <Button variant="outline" className="gap-2" disabled={!resultBatch.length} onClick={() => pushHistory('batch')}>
        ...
        <div key={i} className="p-3 grid gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
          ...
          <div className="flex gap-2 justify-end">
            {Array.isArray(h.result) ? (
              <>
                <Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(h.result.join('\n'))}>
                  <Copy className="h-4 w-4" /> Copy All
                </Button>
