...
export default function WeekNumberClient() {
 ...
 const gotoDelta = React.useCallback(
    (deltaWeeks: number) => {
      setDateStr(
        fmtDateInput(
          new Date(date.getFullYear(), date.getMonth(), date.getDate() + deltaWeeks * 7),
        ),
      );
    },
    [date, fmtDateInput, setDateStr],
  );

  const setToday = React.useCallback(() => {
    setDateStr(fmtDateInput(new Date()));
  }, [fmtDateInput, setDateStr]);

  const latestSummaryRef = React.useRef(summary);
  React.useEffect(() => {
  ...
  const handleKey = React.useCallback(
    (e: KeyboardEvent) => {
      ...
      const k = e.key;
      if (k === "ArrowLeft") gotoDelta(-1);
      else if (k === "ArrowRight") gotoDelta(1);
      else if (k === "t" || k === "T") setToday();
      else if (k === "c" || k === "C") {
        void navigator.clipboard.writeText(latestSummaryRef.current ?? "");
      }
    },
    [gotoDelta, setToday],
  );
