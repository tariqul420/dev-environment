...
type FlatItem = ToolItem & {
  category: string;
  categoryUrl: string;
  // Keep as unknown to tolerate bad data; we normalize at render time.
  icon?: unknown;
};
...
function asIcon(maybe: unknown): LucideIcon {
  // lucide icons are functions/components; objects/strings are invalid
  ...
  for (const group of data) {
    ...
    for (const item of group.items ?? []) {
      out.push({
        ...
        icon: group.icon, // may be invalid; we normalize later
      });
    }
  }
  return out;
