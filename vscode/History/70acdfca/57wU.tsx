...
import { Braces, Code2, Copy, Download, Info, RefreshCw, Settings2, Shuffle } from "lucide-react";
...
// Small helpers
function useDebouncedEffect(effect: () => void, deps: React.DependencyList, ms = 250) {
  React.useEffect(() => {
    const t = setTimeout(effect, ms);
...
export default function YamlJsonClient() {
  // libs
  const [yamlLib, setYamlLib] = React.useState<null | { load: any; dump: any; loadAll?: any }>(
    null,
  );
  React.useEffect(() => {
    let mounted = true;
    (async () => {
      ...
      if (mounted) setYamlLib({ load: mod.load, dump: mod.dump, loadAll: (mod as any).loadAll });
    })();
  ...
  const toJson = React.useCallback(
    (obj: any) => JSON.stringify(normalizeKeys(obj), null, Math.max(0, jsonSpaces)),
    ...
    try {
      ...
      if (dir === "yaml-to-json") {
        ...
        if (yamlMultiDocs && yamlLib.loadAll) {
          const docs: any[] = [];
          yamlLib.loadAll(input, (doc: any) => docs.push(doc));
      ...
      <GlassCard>
        <CardContent className="grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
          ...
          <div className="rounded-lg border p-3 space-y-3">
            ...
            {/* Quick file importers */}
            <div className="grid gap-2">
              <div className="text-xs font-medium opacity-80">Quick Import</div>
              <InputField
                type="file"
