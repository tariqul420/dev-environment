              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <ResetButton onClick={() => setLogoDataUrl(null)} disabled={!logoDataUrl} />
                  </div>
                </div>
        ...
        <GlassCard className="shadow-sm">
          ...
          <CardContent className="grid gap-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              ...
              <Button className="w-full" variant="outline" onClick={() => downloadSVG('qrcode.svg')} disabled={(format ?? 'png') !== 'svg'}>
                <ArrowDownToLine className="mr-2 h-4 w-4" />
                Download SVG
              </Button>
            </div>
...
function DynamicFields({ form, setForm, controlForm }: { form: FormState; setForm: React.Dispatch<React.SetStateAction<FormState>>; controlForm: ReturnType<typeof useForm<ControlValues>> }) {
  if (form.kind === 'url') {
    return (
      <div className="space-y-2">
        <InputField id="url" label="URL" placeholder="https://example.com" value={form.url} onChange={(e) => setForm((s) => ({ ...s, url: e.target.value }))} />
      </div>
    );
  }
  ...
  if (form.kind === 'text') {
    return (
      <TextareaField id="text" label="Text" placeholder="Your message" value={form.text} onValueChange={(v) => setForm((s) => ({ ...s, text: v }))} rows={3} autoResize showCount maxLength={1000} />
    );
  }
    ...
    return (
      <Form {...controlForm}>
        <form className="grid gap-4 sm:grid-cols-2">
          <InputField id="ssid" label="SSID" placeholder="MyNetwork" value={form.wifiSsid} onChange={(e) => setForm((s) => ({ ...s, wifiSsid: e.target.value }))} />
          ...
          {form.wifiAuth !== 'nopass' && (
            <InputField id="wifipw" label="Password" placeholder="supersecret" value={form.wifiPassword} onChange={(e) => setForm((s) => ({ ...s, wifiPassword: e.target.value }))} />
          )}
  ...
  if (form.kind === 'vcard') {
    return (
      <div className="grid gap-4 sm:grid-cols-2">
        <InputField id="vcfirst" label="First Name" value={form.vcFirst} onChange={(e) => setForm((s) => ({ ...s, vcFirst: e.target.value }))} />
        <InputField id="vclast" label="Last Name" value={form.vcLast} onChange={(e) => setForm((s) => ({ ...s, vcLast: e.target.value }))} />
        <InputField id="vcorg" label="Organization" value={form.vcOrg} onChange={(e) => setForm((s) => ({ ...s, vcOrg: e.target.value }))} />
        <InputField id="vctitle" label="Title" value={form.vcTitle} onChange={(e) => setForm((s) => ({ ...s, vcTitle: e.target.value }))} />
        <InputField id="vcphone" label="Phone" value={form.vcPhone} onChange={(e) => setForm((s) => ({ ...s, vcPhone: e.target.value }))} />
        <InputField id="vcemail" type="email" label="Email" value={form.vcEmail} onChange={(e) => setForm((s) => ({ ...s, vcEmail: e.target.value }))} />
        <div className="col-span-2">
          <InputField id="vcurl" label="Website" value={form.vcUrl} onChange={(e) => setForm((s) => ({ ...s, vcUrl: e.target.value }))} />
        </div>
      </div>
    );
  }
  ...
  if (form.kind === 'email') {
    return (
      <div className="grid gap-4 sm:grid-cols-2">
        <InputField id="mailto" type="email" label="To" placeholder="hello@example.com" value={form.emailTo} onChange={(e) => setForm((s) => ({ ...s, emailTo: e.target.value }))} />
        <InputField id="mailsub" label="Subject" placeholder="Subject" value={form.emailSubject} onChange={(e) => setForm((s) => ({ ...s, emailSubject: e.target.value }))} />
        <TextareaField
          id="mailbody"
          label="Body"
          placeholder="Message..."
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      <InputField id="wato" label="Phone (no +)" placeholder="8801XXXXXXXXX" value={form.waTo} onChange={(e) => setForm((s) => ({ ...s, waTo: e.target.value }))} />
      <TextareaField id="watext" label="Text" placeholder="Message…" value={form.waText} onValueChange={(v) => setForm((s) => ({ ...s, waText: v }))} rows={3} autoResize showCount maxLength={1000} />
    </div>
  );
}
