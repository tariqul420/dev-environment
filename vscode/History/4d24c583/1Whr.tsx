...
import { Calendar, Check, Clock, Copy, Diff, Globe, MapPin, Plus, Replace, Trash2 } from 'lucide-react';
...
/** Turn a wall time in a given IANA zone into an exact UTC timestamp (ms). */
...
/** Short TZ abbr (PDT, BST) fallback to zone id if not available */
...
function getOffsetMinutes(tsUtc: number, timeZone: string): number {
  ...
  // offset = local(Zone) - UTC
  // If zone is ahead of UTC (Dhaka), asUtc > tsUtc (positive)
...
/** Try to fetch all supported IANA zones (modern browsers). Fallback to curated list. */
...
function CityRow({ tz, utcTs, srcTz, is12h, onRemove, onSwap }: { tz: string; utcTs: number; srcTz: string; is12h: boolean; onRemove: () => void; onSwap: () => void }) {
  ...
  const diffLabel = diffMin === 0 ? 'same' : `${diffMin > 0 ? '+' : '−'}${Math.abs(diffMin)}m`;
      ...
      <ToolPageHeader
        ...
        actions={
          <>
            ...
            <ActionButton variant="outline" icon={<Clock className="w-4 h-4" />} label="Now" onClick={applyNow} />
            <CopyButton variant="default" label="Copy Link" copiedLabel="Copied Link" getText={link} />
          </>
        }
      />
      ...
      <GlassCard className="shadow-sm">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div className="space-y-3">
            ...
            <SelectField label="Source City / Time Zone" value={sourceTz} onValueChange={(e) => setSourceTz(e.terget.value)} items={sourceCandidates} placeholder="Select source time zone" />
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div className="space-y-3">
            ...
            <div className="grid gap-2">
              <Input placeholder="Search or paste IANA zone (e.g., Europe/Paris)" value={search} onChange={(e) => setSearch(e.target.value)} />
