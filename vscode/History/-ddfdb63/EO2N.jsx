...
const formSchema = z.object({
  ...
  dlDateOfBirth: z.string().optional(),
  nidDateOfBirth: z.string().optional(),

  typesOfWork: z
    .array(z.string().min(1, "Type of Work is required"))
  ...
  // File URL fields
  nid: z.string().url().optional().or(z.literal("")),
  certificate: z.string().url().optional().or(z.literal("")),
  drivingLicence: z.string().url().optional().or(z.literal("")),
  utilityBill: z.string().url().optional().or(z.literal("")),
  medicalReport: z.string().url().optional().or(z.literal("")),
  doopTestReport: z.string().url().optional().or(z.literal("")),
  visa: z.string().url().optional().or(z.literal("")),
  passport: z.string().url().optional().or(z.literal("")),
  ticket: z.string().url().optional().or(z.literal("")),
});
...
export default function ServiceForm({ service }) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      dlDateOfBirth: service?.dlDateOfBirth
        ? new Date(service.dlDateOfBirth).toISOString().slice(0, 10)
        : "",
      nidDateOfBirth: service?.nidDateOfBirth
        ? new Date(service.nidDateOfBirth).toISOString().slice(0, 10)
        : "",
      ...
      agreementOfAmount: service?.agreementOfAmount ?? 0,
  ...
  const onSubmit = async (values) => {
    const formattedValues = {
      ...values,
      dlDateOfBirth: new Date(values?.dlDateOfBirth),
      nidDateOfBirth: new Date(values?.nidDateOfBirth),
    };
    ...
    toast.promise(
          ...
          }),
      {
        ...
        success: () => {
          router.refresh();
          if (!service) {
            router.push("/admin/brta-services");
          }
          return service ? "Service updated!" : "Service created!";
        },
