...
export default function SimpleInvoiceClient() {
  ...
  const onPrint = () => {
    ...
    const win = window.open("", "print", "noopener,noreferrer,width=1024,height=768");
    if (!win) return;
  ...
  <html>
    <head>
      ...
      <style>
        ...
        /* Improve color accuracy in print */
        * {
          -webkit-print-color-adjust: exact !important;
          print-color-adjust: exact !important;
        }
        /* Reset app background for print */
        html, body {
          background: #fff !important;
        }
        /* Avoid breaking the header and totals areas */
        .no-break, .print-no-break { break-inside: avoid; page-break-inside: avoid; }
        /* Table: try not to split rows across pages */
        table, tr, td, th { break-inside: avoid; page-break-inside: avoid; }
        thead { display: table-header-group; }
        tfoot { display: table-footer-group; }

        /* Optional: scale content slightly to fit fewer pages */
        /* :root { --print-scale: .96; } body { zoom: var(--print-scale); } */

        /* If your app uses dark mode, force preview block to white */
        .print-surface {
          background: #fff !important;
          color: #000 !important;
        }
        /* Hide non-preview controls if accidentally included */
        .print-hidden { display: none !important; }
      </style>
    </head>
    <body>
      <div class="print-surface">${node.outerHTML}</div>
    </body>
  </html>`;
    ...
    win.onload = () => {
      // Minor delay helps some browsers compute layout
      setTimeout(() => {
        win.focus();
        win.print();
        win.close();
