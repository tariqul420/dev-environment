...
export const getPaymentById = async (id) => {
  try {
    ...
    // Fetch the payment
    const paymentDoc = await PaymentModel.findById(id).lean();
    if (!paymentDoc) return null;

    const phone = paymentDoc.payerPhone || "";

    // Aggregate totals for the same payerPhone (empty string groups "no phone")
    const [agg] = await PaymentModel.aggregate([
      {
        $match: { payerPhone: phone },
      },
      {
        $group: {
          _id: "$payerPhone",
    ...
    const totalAmount = agg?.totalAmount ? Number(agg.totalAmount) : 0;
    const paymentCount = agg?.paymentCount ? Number(agg.paymentCount) : 0;

    // Return plain object with totals added (keeps your existing caller simple)
    return JSON.parse(
      JSON.stringify({
        ...paymentDoc,
        totalAmount,
