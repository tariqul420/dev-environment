// components/receipt/order-receipt-client.tsx
"use client";
...
// ====== Types (match getOrderReceipt selection) ======
...
// ====== Helpers ======
const asNumber = (n: any) => {
  const x = typeof n === "string" ? Number(n) : Number(n ?? 0);
  return Number.isFinite(x) ? x : 0;
};

const bdt = (n: any) => `BDT ${asNumber(n).toFixed(2)}`;

// Bengali digit → Latin digit
...
// ====== Client wrapper: translate FIRST, then render download ======
export function OrderReceiptClient({ order }: { order: OrderReceiptData }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // translate phone digits (bn → latin), keep as-is otherwise
        const phone = bnToLatinDigits(order.customerPhone);

        // translate each item label (title + optional packageDuration)
        ...
        logger.error("Receipt translate error:", err);
        if (!mounted) return;
  ...
  return (
    <PDFDownloadLink
      // translate first: only feed document once translations are ready
      document={
        ready ? (
