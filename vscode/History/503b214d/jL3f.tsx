...
export type OrderReceiptData = {
  ...
  customerName: string; // Bangla or English
  customerPhone: string; // Bangla or English digits—render as-is
  shippingAddress: string; // Bangla or English
  orderNote?: string | null; // Bangla or English
  referral?: string | null; // Bangla or English

  items: Array<{
    id: string;
    title: string; // Bangla or English
    qty: number;
    unitPrice: number | string;
    total: number | string;
    product?: {
      packageDuration?: string | null; // Bangla or English
      images?: { url: string }[];
    } | null;
  }>;
};
...
export function OrderReceiptButton({ order }: { order: OrderReceiptData }) {
  // Dynamically load react-pdf (web build only)
  ...
  useEffect(() => {
    ...
    import("@react-pdf/renderer")
      .then((m) => {
        if (!mounted) return;
        try {
          // Bengali-capable font (supports English too)
          m.Font.register({
          ...
          // avoid awkward hyphenation in Bangla
  ...
  const styles = useMemo(() => {
    ...
    return PDF.StyleSheet.create({
      ...
      value: {
        fontWeight: "bold",
        textAlign: "right",
        textTransform: "none", // keep exact input (Bangla/English)
      },
  ...
  // Build the PDF document with dynamic module
  const renderDoc = () => {
    ...
    // Mix-language content: we print whatever string comes in (Bangla/English)
