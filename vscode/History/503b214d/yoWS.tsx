...
export function OrderReceiptButton({ order }: { order: OrderReceiptData }) {
  // Load react-pdf on client at runtime (prevents wrong build / SSR issues)
    ...
    import("@react-pdf/renderer")
      .then((m) => {
        ...
        try {
          ...
          // ignore duplicate registrations (HMR)
        }
        setPDF(m);
      })
      .catch((err) =>
  ...
  // Styles created after PDF is ready
  ...
  // Compute item titles (for translation)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [name, address, ...itemsTranslated] = await Promise.all([
          translateToEnglish(order.customerName),
          translateToEnglish(order.shippingAddress),
  ...
  const ready = !!PDF && !!styles && !!t;

  // Build the PDF document with the dynamically loaded module
  const renderDoc = () => {
    if (!PDF || !styles || !t) return null;
