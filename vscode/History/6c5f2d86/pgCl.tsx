...
import { CopyButton, DownloadTextButton, ResetButton } from '@/components/shared/action-buttons';
...
import { History, ListChecks, ListOrdered, Wand2 } from 'lucide-react';
...
export default function TextToListPage() {
  ...
  async function copyList() {
    ...
    setCopiedKind('list');
    setTimeout(() => setCopiedKind(null), 1000);
  }
  ...
  const csvContent = React.useMemo(() => csvFromRows([['Item'], ...processed.map((s) => [s])]), [processed]);
  ...
  return (
    <>
      ...
      <ToolPageHeader
        icon={ListChecks}
        title="Text to List"
        description="Split by comma/newline → clean list with trim, dedupe, sort, case, prefix/suffix & numbering."
        actions={
          <>
            ...
            <DownloadTextButton variant="outline" filename="clean-list.csv" getText={() => csvContent} label="CSV" disabled={!processed.length} />
      ...
      <GlassCard className="shadow-sm">
        ...
        <CardContent className="grid gap-6 md:grid-cols-2">
          <div className="space-y-3">
            ...
            <TextareaField readOnly value={processed.join('\n')} onValueChange={() => {}} textareaClassName="min-h-[200px] font-mono" />
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <DownloadTextButton variant="outline" size="sm" filename="clean-list.csv" getText={() => csvContent} label="CSV" disabled={!processed.length} />
