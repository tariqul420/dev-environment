...
export const getBlogBySlug = async (slug: string, isAdmin: boolean = false) => {
  try {
    await dbConnect();
...
export const incrementBlogViews = async (slug: string, path: string) => {
  await dbConnect();
...
export const updateBlogLike = async (id: string, increment = true, path: string) => {
  await dbConnect();
