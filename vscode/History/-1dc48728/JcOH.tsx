  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      for (const f of files) {
        ...
        // Skip if URL already exists (means it's either uploaded or from selectable images)
          continue;

        // Only upload if it's a newly added file (has File object)
        if (f.file && f.file instanceof File) {
          try {
            ...
            // Update form state with uploaded URL
    ...
    (image: SelectableImage) => {
      ...
      if (multiple) {
        if (currentUrls.includes(image.url)) {
          ...
          // Add to idToUrl mapping for selected image
