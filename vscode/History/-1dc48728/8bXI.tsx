...
export default function ImageUploaderField({
  ...
  const initialFiles: InitialFileMeta[] = useMemo(() => {
    ...
    return urls.map((url, i) => ({
      ...
      type: "image/jpeg",
      url,
      id: `init-${i}-${url}-${forceUpdateKey}`,
    }));
  }, [getList, forceUpdateKey]);
  ...
  // Update idToUrl mapping when form state changes
  useEffect(() => {
    const currentUrls = getList();
    // Update idToUrl mapping for all current URLs
    currentUrls.forEach((url, index) => {
      const correspondingFile = initialFiles[index];
      if (correspondingFile) {
        idToUrl.current[correspondingFile.id] = url;
      }
    });
  }, [getList, initialFiles]);
  ...
  const handleSelectImage = useCallback(
    (image: SelectableImage) => {
      ...
      // Force re-render to sync with new URLs
