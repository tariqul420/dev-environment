...
export default function TechStackCard({ data }: CardProps) {
  return (
    <Link rel="noopener noreferrer" href={data.link} target="_blank">
      <div className="relative group rounded-lg p-6 shadow border-l-2 border-border flex flex-col min-h-full bg-gradient-to-r from-card to-transparent">
        {/* Icon (top right) */}
        <div className="absolute top-4 right-4 cursor-pointer group-hover:-translate-y-1 group-hover:rotate-[50deg] duration-150 transition-all ease-linear">
          <ArrowUpRight className="w-8 h-8 p-2 rounded-full flex items-center justify-center bg-muted" />
        </div>
