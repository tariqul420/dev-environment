...
import { Calculator, Copy, Info, Timer } from "lucide-react";
          ...
          {mode === "salary" ? (
            <div className="grid gap-4 sm:grid-cols-3">
              <ResultBox
                ...
                onCopyText={fmtMoney(results.hourly, currency, round2)}
              />
              <ResultBox
                label="Daily"
                value={fmtMoney(results.daily, currency, round2)}
                onCopyText={fmtMoney(results.daily, currency, round2)}
              />
              <ResultBox
                label="Weekly"
                value={fmtMoney(results.weekly, currency, round2)}
                onCopyText={fmtMoney(results.weekly, currency, round2)}
              />
              <ResultBox
                label="Monthly"
                value={fmtMoney(results.monthly, currency, round2)}
                onCopyText={fmtMoney(results.monthly, currency, round2)}
              />
              <ResultBox
                label="Annual"
                value={fmtMoney(results.annual, currency, round2)}
                onCopyText={fmtMoney(results.annual, currency, round2)}
              />
              {ptoDays > 0 && countPaidPTO && (
                <ResultBox
                  label="Hourly (effective w/ PTO)"
                  value={fmtMoney(results.hourlyEffective, currency, round2)}
                  onCopyText={fmtMoney(results.hourlyEffective, currency, round2)}
                />
              )}
            </div>
          ) : (
