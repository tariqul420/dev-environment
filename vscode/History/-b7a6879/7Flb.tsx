...
import { GripVertical as IconGripVertical, ImageUp, Trash2 } from 'lucide-react';
...
function SortableImageItem({ url, index, onDelete }: { url: string; index: number; onDelete: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="group relative overflow-hidden rounded-md border">
      {valid ? (
        <Image src={url} alt={`Uploaded image ${index + 1}`} width={300} height={200} className="h-[200px] w-full object-cover" />
      ) : (
        <div className="text-muted-foreground flex h-[200px] w-full items-center justify-center text-xs">Invalid image URL</div>
      )}
...
export default function ImageUploaderField({ name, label, multiple = false, className, viewClass }: Props) {
  ...
  const getList = (): string[] => ((getValues(name) as string[]) || []).filter((u) => !!u && u.trim() !== '');
          ...
          <FormControl>
            <div className={cn('grid grid-cols-2 gap-4', viewClass)}>
              {multiple ? (
                <>
                  <DndContext onDragEnd={handleDragEnd}>
                    <SortableContext items={getList()} strategy={verticalListSortingStrategy}>
                  ...
                  {(() => {
                    ...
                    return valid ? (
                      <div className="group relative overflow-hidden rounded-md border">
                        <Image src={val} alt="Uploaded image" width={300} height={200} className="h-[200px] w-full object-cover" />
              ...
              <CldUploadWidget
                ...
                {({ open }) => (
                  <div className="hover:bg-muted flex h-[200px] cursor-pointer flex-col items-center justify-center rounded-md border border-dashed p-4 transition" onClick={() => open?.()}>
                    <ImageUp className="text-muted-foreground h-6 w-6" />
                    <p className="text-muted-foreground mt-2 text-xs">{multiple ? 'Upload or drag here' : getValues(name) ? 'Replace with another image' : 'Upload or drag here'}</p>
                    <span className="text-muted-foreground text-[10px]">JPG, PNG — max 300×200</span>
                  </div>
                )}
              </CldUploadWidget>
            </div>
