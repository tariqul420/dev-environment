                ...
                <>
                  {((getValues(name) as string[]) || []).map((url, index) => (
                    <div key={index} className="relative group border rounded-md overflow-hidden">
                      <Image src={url} alt={`Uploaded image ${index + 1}`} width={300} height={200} className="w-full h-[200px] object-cover" />
                      <Button
                        type="button"
                        variant="ghost"
                        size="icon"
                        ...
                        onClick={() => {
                          const updated = (getValues(name) as string[]).filter((_, i) => i !== index);
              ...
              <CldUploadWidget
                ...
                {({ open }) => (
                  <div className="flex flex-col items-center justify-center border border-dashed rounded-md p-4 cursor-pointer hover:bg-muted transition h-[200px]" onClick={() => open?.()}>
                    ...
                    <p className="text-xs mt-2 text-muted-foreground">{multiple ? 'Upload or drag here' : getValues(name) ? 'Replace with another image' : 'Upload or drag here'}</p>
