...
import { Copy, Eraser, Filter, Hash, List, ListOrdered, Replace, Scissors, Search, Shuffle, SortAsc, SortDesc } from 'lucide-react';
...
export default function LineToolsClient() {
  ...
  // always apply affixes/numbering to the last operation result
  ...
  }, [resultLines, numbering, startNum, numSep, prefix, suffix]);
    ...
    <>
      ...
      <GlassCard className="shadow-sm">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <DownloadTextButton filename="lines-output.txt" getText={() => output} disabled={!output} />
            ...
            <CopyButton getText={() => output || ''} disabled={!output} label="Copy Output" />
            <Button variant="outline" className="gap-2" onClick={() => setText(output)} disabled={!output}>
              <Replace className="h-4 w-4" /> Replace Input
            </Button>
            <Button variant="outline" className="gap-2" onClick={() => setResultLines([])} disabled={!output}>
