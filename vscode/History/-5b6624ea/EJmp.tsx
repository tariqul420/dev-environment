...
import { ChevronRight, ExternalLink, FolderKanban, LayoutDashboard, MoreVertical, Newspaper, ShieldCheck, Sparkles } from 'lucide-react';
...
export function DashboardSidebar({ menu = [], user }: DashboardSidebarProps) {
  ...
  // Active matcher: exact for /admin, prefix for others
    ...
    <TooltipProvider>
      <Sidebar className="bg-gradient-to-b from-background to-muted/30 backdrop-blur supports-[backdrop-filter]:bg-background/80">
        ...
        <SidebarContent>
          ...
          <SidebarGroup>
            <SidebarGroupLabel className="text-xs font-medium tracking-wide text-muted-foreground">Shortcuts</SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu>
                <SidebarMenuItem>
                  <SidebarMenuButton asChild className="rounded-xl hover:bg-muted/60">
          ...
          <div className="mx-3 mt-2 rounded-xl border bg-muted/40 p-3">
        ...
        <SidebarFooter className="border-t bg-muted/40 p-3">
          ...
          <div className="mt-2 flex items-center justify-between text-[10px] text-muted-foreground">
            <span className="inline-flex items-center gap-1">
              <ExternalLink className="h-3 w-3" /> <span>v2.1</span>
            </span>
            <span>© {new Date().getFullYear()} Tariqul</span>
          </div>
        </SidebarFooter>
      </Sidebar>
    </TooltipProvider>
