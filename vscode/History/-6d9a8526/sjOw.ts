...
export const updateUserRole = async ({
  ...
  if (!updated) return { ok: false, error: "Failed to update user role" };

   if (!updated.clerkUserId) {
    // Not fatal, but report
    console.warn("User has no clerkUserId; skipping Clerk metadata sync", userId);
  } else {
  ...
  // Invalidate caches (use your own tags if you have them)
