...
function genRobots(s: State) {
  ...
  s.agents.forEach((ag, i) => {
    ...
    ag.rules.forEach((r) => {
      ...
      if (!p && r.allow) return;
      if (!p && !r.allow)
        // avoid "Allow: " with empty path
        return;
      if (r.allow)
        if (r.allow)
          // empty Disallow means allow all; skip here
          allowLines.push(`Allow: ${p}`);
  ...
  if (sm.length) {
    if (lines.length && lines.at(-1) !== "") lines.push("");
    sm.forEach((u) => lines.push(`Sitemap: ${u}`));
  }
      ...
      <ToolPageHeader
        ...
        actions={
          <>
            ...
            <CopyButton />
            <ActionButton icon={Copy} label="Copy" variant="outline" onClick={copyOut} />
            <ActionButton
              icon={Download}
              label="Download"
              onClick={() => downloadTxt("robots.txt", output)}
            />
          </>
        }
      />
