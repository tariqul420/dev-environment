...
export default function TimestampConverterClient() {
 ...
 return (
  <>
    ...
    <GlassCard>
      {/* 1 col on mobile, 2 cols from md; keep nice ratio on lg */}
      <CardContent className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:[grid-template-columns:1.1fr_1fr]">
        {/* Left: Inputs */}
        <div className="rounded-xl border p-4 space-y-4">
          ...
          {dir === "toDate" ? (
            ...
            <div className="flex max-sm:flex-col max-sm:items-stretch sm:items-end gap-3">
              <InputField
                className="w-full"
                id="epoch"
              ...
              />
              <div className="flex max-sm:w-full sm:w-auto items-center gap-1">
                <ActionButton
                  label="Now"
          ...
          {/* Action row: wrap and stretch on mobile */}
        ...
        <div className="rounded-xl border p-4">
          <h3 className="mb-2 text-xs sm:text-sm font-medium tracking-wide uppercase text-muted-foreground">
            Result
          </h3>

          {!error && !Object.keys(result).length ? (
            <div className="rounded-md border p-3 text-xs sm:text-sm text-muted-foreground">
              Enter a value and tap <b>Convert</b>. Use <b>Now</b> for quick checks.
            </div>
          ) : null}

          {error ? (
            <div className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-xs sm:text-sm text-destructive">
              {error}
            </div>
          ) : (
            // Make results scrollable on small screens, keep line clamping readable
            <div className="grid gap-2 max-h-[50vh] overflow-auto pr-1">
              {Object.entries(result).map(([k, v]) => (
                ...
                >
                  <div className="min-w-[120px] sm:min-w-[160px] text-[11px] sm:text-xs text-muted-foreground">
                    {k}
                  </div>
                  <div className="flex-1 truncate font-mono text-xs sm:text-sm">{v}</div>
