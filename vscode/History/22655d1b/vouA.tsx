...
const formSchema = z.object({
  ...
  categories: z.array(z.string().min(1, { message: 'Category is required.' })).min(1, { message: 'Select at least one category.' }),
...
export default function ProjectForm({ project }: ProjectFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      categories: Array.isArray(project?.categories) ? project.categories.filter((v): v is string => !!v) : [],
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-8">
        ...
        <MultiSelectField
          ...
          options={[
            ...
            // 🔥 নতুন অ্যাড করা ক্যাটেগরি গুলো
            { label: 'NGO / Non-Profit', value: 'ngo' },
            { label: 'Digital Marketing', value: 'digital-marketing' },
            { label: 'Business / Corporate Website', value: 'business' },
            { label: 'Educational / LMS', value: 'education' },
            { label: 'Health & Fitness', value: 'health' },
            { label: 'Finance / FinTech', value: 'finance' },
            { label: 'News & Magazine', value: 'news' },
            ...
            { label: 'Marketplace / Multi-vendor', value: 'marketplace' },
            { label: 'Personal Blog', value: 'personal-blog' },
            { label: 'Agency / Service', value: 'agency' },
          ]}
        />
