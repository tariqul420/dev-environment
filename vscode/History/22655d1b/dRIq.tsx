...
export default function ProjectForm({ project }: ProjectFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      tags: project?.tags || [],
      launchDate: project?.launchDate?.toISOString().substring(0, 10) || '',
      category: typeof project?.category === 'object' ? project.category.toString() : project?.category || '',
  ...
  const onSubmit = async (values: FormValues) => {
    const formattedValues: IProject = {
      ...values,
      launchDate: new Date(values.launchDate), // ✅ convert string to Date
    };

    toast.promise(project ? updateProject({ projectId: project._id, data: formattedValues, path: pathname }) : createProject({ data: formattedValues }), {
