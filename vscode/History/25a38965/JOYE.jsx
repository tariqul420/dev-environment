...
// ⬇️ You'll implement these actions similar to project.action
...
const formSchema = z.object({
  ...
  status: z.enum(["pending", "success", "failed", "refunded", "disputed"]).default("pending"),
  ...
  receipts: z.array(z.string().url()).optional(), // image URLs
});
...
export default function PaymentForm({ payment }) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      paidAt: payment?.paidAt
        ? new Date(payment.paidAt).toISOString().slice(0, 16) // yyyy-MM-ddTHH:mm
        : "",
  ...
  const onSubmit = async (values) => {
    const netAmount = Number(values.amount) - Number(values.fees || 0);
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        <InputField
          name="netAmountPreview"
          label="Net Amount (auto)"
          value={isFinite(netAmountLive) ? netAmountLive.toFixed(2) : "0.00"}
          disabled
        />
