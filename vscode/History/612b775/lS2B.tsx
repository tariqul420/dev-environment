...
import { AlignLeft, BarChart2, Clock4, FileText, Info, Keyboard, Mic, Type as TypeIcon } from 'lucide-react';
...
function QuickTransforms({ toUpper, toLower, toTitle, toSlug, cleanSpaces }: { toUpper: () => void; toLower: () => void; toTitle: () => void; toSlug: () => void; cleanSpaces: () => void }) {
  const actions: Action[] = [
    { key: 'upper', label: 'UPPERCASE', run: toUpper, hotkey: 'U' },
    { key: 'lower', label: 'lowercase', run: toLower, hotkey: 'L' },
    { key: 'title', label: 'Title Case', run: toTitle, hotkey: 'T' },
    { key: 'slug', label: 'slugify', run: toSlug, hotkey: 'S' },
    { key: 'clean', label: 'Remove extra spaces & blank lines', run: cleanSpaces, hotkey: 'R', span2: true },
  ];
  ...
  React.useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      const match = actions.find((a) => a.hotkey?.toLowerCase() === pressed);
      if (match) {
        e.preventDefault();
        match.run();
      }
    ...
    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
      {actions.map((a) => (
        <Button key={a.key} variant="outline" className={cn('justify-start gap-2', a.span2 && 'sm:col-span-2')} onClick={a.run}>
          ...
          {a.label}
          {a.hotkey && (
            <kbd className="ml-auto inline-flex items-center gap-1 rounded border bg-muted px-1.5 py-0.5 text-[10px] tracking-wider text-muted-foreground">
              <Keyboard className="h-3 w-3" /> {a.hotkey}
            </kbd>
          )}
        </Button>
      ))}
    ...
    <>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <GlassCard className="p-5">
          ...
          <Textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Start typing or paste your text here…" className="mt-2  font-mono" />
