...
import {
  type LoadedImage,
  loadImageMeta as loadMeta,
  useImageInput,
} from "@/hooks/use-image-input";
import {
  createImageElement,
  formatBytes,
  type OutFormat,
...
async function reencodeWithoutMetadata(opts: {
  ...
  orientation?: number;
  background?: string; // used for JPEG when transparency found
}): Promise<{ blob: Blob }> {
  ...
  // If output is JPEG and background given (to replace transparency)
    // Draw onto a new canvas with bg
...
function getTagValue(
  ...
  switch (type) {
    ...
    case 5: {
      // RATIONAL
        return [dv.getUint32(valuePtr, little), dv.getUint32(valuePtr + 4, little)];
    ...
    case 10: {
      // SRATIONAL
        return [dv.getInt32(valuePtr, little), dv.getInt32(valuePtr + 4, little)];
      const arr: [number, number][] = [];
      for (let i = 0; i < count; i++) {
        const off = valuePtr + i * 8;
    ...
    case 9: {
      // SLONG
        return dv.getInt32(valuePtr, little);
      const arr: number[] = [];
