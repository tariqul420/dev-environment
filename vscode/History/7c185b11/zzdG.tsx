...
function getTagValue(dv: DataView, tiffBase: number, type: number, count: number, valueOffset: number, little: boolean): any {
  ...
  const valuePtr = valueBytes > 4 ? get32(valueOffset, little) + tiffBase : valueOffset;
  ...
  switch (type) {
    ...
    case 3: {
      // SHORT
      if (count === 1) return get16(valuePtr, little);
      const arr: number[] = [];
      for (let i = 0; i < count; i++) arr.push(get16(valuePtr + i * 2, little));
      return arr;
    }
    case 4: {
      // LONG
      if (count === 1) return get32(valuePtr, little);
      const arr: number[] = [];
      for (let i = 0; i < count; i++) arr.push(get32(valuePtr + i * 4, little));
      return arr;
    }
    case 5: {
      // RATIONAL
      ...
      for (let i = 0; i < count; i++) arr.push([dv.getUint32(valuePtr + i * 8, little), dv.getUint32(valuePtr + i * 8 + 4, little)]);
      return arr;
    }
    case 10: {
      // SRATIONAL
      ...
      const arr: [number, number][] = [];
      for (let i = 0; i < count; i++) arr.push([dv.getInt32(valuePtr + i * 8, little), dv.getInt32(valuePtr + i * 8 + 4, little)]);
      return arr;
    }
    case 1:
    case 7: {
