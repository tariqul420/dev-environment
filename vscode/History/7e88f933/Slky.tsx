...
export type QrLogo = {
  ...
  sizePct?: number; // % of min(width,height), default 20
  roundedPct?: number; // corner radius as % of logo size, default 20
  pad?: number; // white padding around logo (px), default 4
};

export type QrProps = {
  value: string;
  format?: QrFormat; // 'png' | 'svg'  (default 'png')
  size?: number; // px (default 200)
  margin?: number; // quiet zone in modules (default 1)
  ecl?: QrECL; // default 'M'
  fg?: string; // default '#000000'
  bg?: string; // default '#ffffff'
  quietZone?: boolean; // default true; if false, margin=0
  logo?: QrLogo | null; // optional center logo
  className?: string;
  canvasClassName?: string;
  svgClassName?: string;
  /** PNG only: returns last rendered dataURL */
