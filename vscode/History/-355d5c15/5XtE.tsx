...
import { ArrowsLeftRight, Check, ClipboardCopyIcon as Copy, Download, FileDown, FileUp, Image as ImageIcon, Link2, RefreshCw, RotateCcw, Settings2, ShieldCheck, Zap } from 'lucide-react';

type Mode = 'encode' | 'decode';

const STORAGE_KEY = 'toolshub.base64.v1';

function clsx(...arr: Array<string | false | undefined>) {
  return arr.filter(Boolean).join(' ');
}

function downloadBlob(filename: string, content: string | BlobPart, type: string) {
...
function encodeTextToBase64(s: string): Uint8Array {
...
export default function Base64ToolPage() {
  ...
  const handleFile = async (file: File) => {
    ...
    setError(null);
    // read as bytes for base64 (not data URL — we decide header later)
    const buf = new Uint8Array(await file.arrayBuffer());
    const b64 = bytesToBase64(buf);
    const b64Final = urlSafe ? toUrlSafe(b64, keepPadding) : b64;
    const result = useDataUri ? makeDataUri(file.type || 'application/octet-stream', b64Final) : b64Final;
    setInput(result);
    setMode('decode'); // encourages trying both ways
  };
  ...
  const encode = () => {
    try {
      ...
      setImagePreview(null);
      // treat input as text
      const bytes = encodeTextToBase64(input);
    ...
    <MotionGlassCard>
      ...
      <GlassCard className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Encode & decode text/files, URL-safe mode, line-wrapping & data URIs. Drag & drop supported.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" className="gap-2" onClick={swapInOut}>
            <ArrowsLeftRight className="h-4 w-4" /> Swap
          </Button>
        ...
        <CardContent className="grid gap-4 lg:grid-cols-4">
          ...
          <div className="rounded-lg border p-3 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={mimeType} onChange={(e) => setMimeType(e.target.value)} placeholder="e.g. image/png or text/plain;charset=utf-8" />
              <p className="text-xs text-muted-foreground">Used when building data URIs during encode or download.</p>
            </div>
          </div>
          ...
          <div className="rounded-lg border p-3 space-y-3">
            ...
            <div className="space-y-1.5">
              <Label>Width</Label>
              <Input type="number" min={4} max={200} value={wrapWidth} onChange={(e) => setWrapWidth(clampInt(e.target.value, 4, 200))} />
              <p className="text-xs text-muted-foreground">For visual formatting (MIME style). Not applied to data URI prefix.</p>
            </div>
          </div>
          ...
          <div className="rounded-lg border p-3 space-y-3">
            ...
            {fileInfo && (
              <p className="text-xs text-muted-foreground">
                <b>{fileInfo.name}</b> • {(fileInfo.size / 1024).toFixed(1)} KB • {fileInfo.type || 'application/octet-stream'}
              </p>
            )}
            {imagePreview && (
      ...
      <GlassCard className="shadow-sm">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Drag & drop any file onto this area to auto-fill input with Base64.</CardDescription>
        </CardHeader>
        <CardContent onDragOver={(e) => e.preventDefault()} onDrop={onDrop}>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="flex flex-col rounded-lg border overflow-hidden">
              <div className="flex items-center justify-between border-b bg-muted/40 px-3 py-2">
                <div className="text-xs font-medium tracking-wide uppercase text-muted-foreground">{mode === 'encode' ? 'Plain Text (to encode)' : 'Base64 / Data URI (to decode)'}</div>
                <div className="flex items-center gap-2">
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button size="sm" variant="outline" className="gap-2" onClick={() => copy('IN')}>
                          {copied === 'IN' ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />} Copy
                        </Button>
                      </TooltipTrigger>
              ...
              <Textarea
                ...
                spellCheck={false}
                placeholder={mode === 'encode' ? 'Type or paste text here…' : 'Paste Base64 or data:[mime];base64,... here…'}
              />
            ...
            <div className="flex flex-col rounded-lg border overflow-hidden">
              <div className="flex items-center justify-between border-b bg-muted/40 px-3 py-2">
                <div className="text-xs font-medium tracking-wide uppercase text-muted-foreground">{mode === 'encode' ? (useDataUri ? 'Base64 (Data URI)' : 'Base64') : 'Decoded Text'}</div>
                <div className="flex items-center gap-2">
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button size="sm" variant="outline" className="gap-2" onClick={() => copy('OUT')}>
              ...
              <Textarea ref={outRef} readOnly value={output} className="min-h-[300px] rounded-none border-0 focus-visible:ring-0 font-mono text-sm" placeholder="Result will appear here…" />
              <div className="flex items-center justify-between border-t bg-muted/30 px-3 py-2 text-xs">
                <span className="text-muted-foreground">
                  Bytes: <b>{bytesOut}</b>
                </span>
                <div className="flex items-center gap-2">
                  {mode === 'encode' ? (
                    <Button size="sm" variant="ghost" className="gap-2" onClick={() => downloadBlob('base64.txt', output, 'text/plain;charset=utf-8')}>
          ...
          {error && <div className="mt-4 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm">{error}</div>}
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between rounded-lg border bg-muted/30 px-3 py-2 text-xs">
            ...
            <div className="flex gap-2">
              <Button size="sm" variant="outline" className="gap-2" onClick={() => setMode('encode')}>
                <ShieldCheck className="h-4 w-4" /> Encode
              </Button>
              <Button size="sm" variant="outline" className="gap-2" onClick={() => setMode('decode')}>
