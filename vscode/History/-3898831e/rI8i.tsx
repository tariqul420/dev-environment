...
import { ActionButton, CopyButton, ExportTextButton } from "@/components/shared/action-buttons";
/* Reusable fields */
...
function buildRows(s: State): Row[] {
  ...
  for (const line of lines) {
    ...
    if (!/^https?:\/\//i.test(normalized)) continue; // absolute HTTP(S) only
...
function toBytes(s: string) {
  return new TextEncoder().encode(s).length; // rough utf-8 size
}
  ...
  return (
    <>
      ...
      <GlassCard className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {preview && (
            <>
              <ActionButton
                icon={copied === "preview" ? Check : Copy}
                label="Copy Preview"
                variant="outline"
      ...
      <ToolPageHeader
        ...
        actions={
          <>
            <CopyButton getText={preview.xml} />
            <ExportTextButton
              label="Download"
              getText={() => preview.xml}
              ...
              {preview && (
                <>
                  <ActionButton
                    variant="outline"
                    size="sm"
                    icon={copied === "preview2" ? Check : Copy}
                    label="Copy"
