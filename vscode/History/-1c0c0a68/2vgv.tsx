...
const FilterBar = ({
  ...
  const isBlogPath = pathName.startsWith('/blogs');
    ...
    <div className="mb-6 flex min-h-[60px] flex-col justify-between rounded border-b pb-4 md:flex-row">
      <div className="left-content order-2 mt-5 flex items-center justify-between gap-4 md:order-1 md:mt-0">
        <Sheet>
          ...
          <SheetContent side="left" className="z-50 block w-[300px] px-2.5 sm:w-[540px] md:px-5 lg:hidden">
            <div className="h-full overflow-y-auto">
              {isBlogPath && project && (
                <div className="mb-8">
                  <SheetHeader>
                    <SheetTitle className="font-grotesk text-xl font-semibold uppercase">FEATURED project</SheetTitle>
                  </SheetHeader>
                  <div className="flex flex-col gap-4">
                    {project.map((product: ProductProps) => (
                      <Link key={product.slug} href={`/project/${product.slug}`} className="group flex items-center gap-4 overflow-hidden rounded-lg border p-2 shadow transition-all hover:shadow-sm">
                        <div className="h-20 w-20 flex-shrink-0 overflow-hidden rounded-md">
                          <Image src={product.imageUrls[0]} alt={product.title} width={80} height={80} className="h-full w-full object-cover transition-transform group-hover:scale-105" />
                        </div>
                        <div className="flex-1">
                          <h3 className="text-md mb-1 line-clamp-2 font-medium">
                            <span className="en text-md font-semibold">{product.title}</span> - <span className="text-base font-medium">{product.titleBengali}</span>
                          </h3>
                          <p className="en mb-auto">
                            <span className="line-through">{product.regularPrice} ৳</span> - <span className="text-accent-main text-lg font-bold">{product.salePrice} ৳</span>
                          </p>
                        </div>
                      </Link>
                    ))}
              ...
              <div className="font-grotesk">
                <SheetHeader>
                  <SheetTitle className="text-xl font-semibold uppercase">PRODUCT CATEGORIES</SheetTitle>
                </SheetHeader>
                <ul className="space-y-3">
                  {categories.map((category: { name: string; slug: string }) => (
                    <li key={category.slug} className="hover:text-accent-hover">
                      <SheetClose asChild>
                        <Link href={`/project?category=${category.slug}`}>{category.name}</Link>
                      </SheetClose>
                    </li>
                  ))}
                </ul>
        ...
        <BreadcrumbContainer
          items={[
            { label: 'Home', href: '/' },
            ...(pathName === '/project' ? [{ label: 'Shop', href: '/project' }] : []),
