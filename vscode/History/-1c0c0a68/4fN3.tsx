    ...
    <div className="mb-6 flex min-h-[60px] flex-col justify-between rounded border-b pb-4 md:flex-row">
      <div className="left-content order-2 mt-5 flex items-center justify-between gap-4 md:order-1 md:mt-0">
          ...
          <SheetContent side="left" className="z-50 block w-[300px] px-2.5 sm:w-[540px] md:px-5 lg:hidden">
            <div className="h-full overflow-y-auto">
              {isBlogPath && products && (
                <div className="mb-8">
                  <SheetHeader>
                    <SheetTitle className="font-grotesk text-xl font-semibold uppercase">FEATURED PRODUCTS</SheetTitle>
                  </SheetHeader>
                  <div className="flex flex-col gap-4">
                    {products.map((product: ProductProps) => (
                      <Link key={product.slug} href={`/products/${product.slug}`} className="group flex items-center gap-4 overflow-hidden rounded-lg border p-2 shadow transition-all hover:shadow-sm">
        ...
        <BreadcrumbContainer
          items={[
            { label: 'Home', href: '/' },
            ...(pathName === '/products' ? [{ label: 'Shop', href: '/products' }] : []),
      ...
      <div className="right-content order-1 flex flex-col items-start gap-2 sm:flex-row sm:items-center md:order-2 md:gap-5">
        ...
        {(pathName === '/products' || pathName === '/blogs') && (
          <>
