...
// Modern Admin Dashboard for tariqul.dev
// - Sleek layout with left sidebar, sticky topbar, and responsive cards
// - Built entirely with Shadcn UI + Tailwind (no chart libs)
// - Drop into: app/(dashboard)/admin/page.tsx
// - Make sure Shadcn components used here are installed (see note at bottom)
...
// ———————————————————————————————————————————————————————
// Mock data (swap with your server queries)
// ———————————————————————————————————————————————————————
const STAT_CARDS = [
...
const RECENT_PROJECTS = [
  { name: 'Meetora — Video Meetings', status: 'Published' as const, lastDeploy: '2h ago', views: 4321 },
...
export default function AdminDashboardPage() {
  ...
  return (
    <TooltipProvider>
      <div className="flex min-h-[calc(100dvh-0px)] bg-gradient-to-b from-background via-background to-background/30">
        {/* ——— Sidebar (desktop) */}
        <aside className="sticky top-0 hidden h-screen w-64 shrink-0 border-r bg-background/80 backdrop-blur lg:block">
          <div className="flex h-14 items-center gap-2 px-4">
            ...
            <div>
              <p className="text-sm font-semibold leading-tight">tariqul.dev</p>
              <p className="text-[11px] text-muted-foreground">Admin</p>
            </div>
          </div>
          <Separator />
          <nav className="px-2 py-4 text-sm">
            <SidebarLink href="/admin" icon={<Rocket className="h-4 w-4" />}>
              Dashboard
            </SidebarLink>
            <SidebarLink href="/admin/projects" icon={<FolderGit2 className="h-4 w-4" />}>
              Projects
            </SidebarLink>
            <SidebarLink href="/admin/blog" icon={<FileText className="h-4 w-4" />}>
              Blog
            </SidebarLink>
            <SidebarLink href="/admin/messages" icon={<Mail className="h-4 w-4" />}>
              Messages
            </SidebarLink>
            <SidebarLink href="/admin/analytics" icon={<BarChart3 className="h-4 w-4" />}>
              Analytics
            </SidebarLink>
            ...
            <div className="mt-6 rounded-xl border p-3">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <ShieldCheck className="h-3.5 w-3.5" /> Protected by middleware
              </div>
            </div>
          </nav>
        </aside>
        ...
        <Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}>
          <SheetTrigger asChild>
            <Button variant="ghost" size="icon" className="lg:hidden absolute left-2 top-2 z-20">
          ...
          <SheetContent side="left" className="w-72 p-0">
            <SheetHeader className="px-4 py-3">
              <SheetTitle className="flex items-center gap-2">
                <div className="h-6 w-6 rounded-lg bg-primary/10" />
                <span>tariqul.dev</span>
              </SheetTitle>
            </SheetHeader>
            <Separator />
            <nav className="px-2 py-4 text-sm">
              <SidebarLink href="/admin" icon={<Rocket className="h-4 w-4" />} onClick={() => setSidebarOpen(false)}>
                Dashboard
              </SidebarLink>
              <SidebarLink href="/admin/projects" icon={<FolderGit2 className="h-4 w-4" />} onClick={() => setSidebarOpen(false)}>
                Projects
              </SidebarLink>
              <SidebarLink href="/admin/blog" icon={<FileText className="h-4 w-4" />} onClick={() => setSidebarOpen(false)}>
                Blog
              </SidebarLink>
              <SidebarLink href="/admin/messages" icon={<Mail className="h-4 w-4" />} onClick={() => setSidebarOpen(false)}>
                Messages
              </SidebarLink>
              <SidebarLink href="/admin/analytics" icon={<BarChart3 className="h-4 w-4" />} onClick={() => setSidebarOpen(false)}>
                Analytics
              </SidebarLink>
              <SidebarLink href="/admin/settings" icon={<Settings className="h-4 w-4" />} onClick={() => setSidebarOpen(false)}>
                Settings
              </SidebarLink>
            </nav>
          </SheetContent>
        ...
        <div className="flex w-full flex-col">
          ...
          <header className="sticky top-0 z-10 border-b bg-background/80 backdrop-blur">
            <div className="flex h-14 items-center gap-2 px-4">
              <div className="ml-10 flex w-full items-center justify-between lg:ml-0">
                <div className="flex items-center gap-2">
                  <div className="hidden rounded-xl border px-2.5 py-1 text-xs text-muted-foreground md:flex items-center gap-1">
                    ...
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="sm" className="gap-2">
                        <Avatar className="h-6 w-6">
                          <AvatarImage src={`https://api.dicebear.com/8.x/identicon/svg?seed=Tariqul`} />
                          <AvatarFallback>TI</AvatarFallback>
                        </Avatar>
                        <span className="hidden md:inline">Tariqul</span>
          ...
          {/* Content */}
          <main className="mx-auto w-full max-w-screen-2xl space-y-6 p-4 md:p-6">
            {/* Intro + Quick Actions */}
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex w-full items-center gap-2 md:w-96">
                    <div className="relative w-full">
                      ...
                      <Input className="pl-8" placeholder="Search messages, projects, posts..." value={search} onChange={(e) => setSearch(e.target.value)} />
                    </div>
                  </div>
                </div>
            ...
            <Card className="border-dashed">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-base">
                  <Star className="h-4 w-4" /> Activity
                </CardTitle>
                <CardDescription>Latest notable events</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <ActivityItem title="Deployed Portfolio v2.1" meta="2h ago • commit a1b2c3" />
                  <ActivityItem title="New message from Nabila" meta="Aug 20, 2025 • collaboration" />
                  <ActivityItem title="Post draft created: Scaling Next.js on VPS" meta="Aug 18, 2025 • blog" />
                </div>
              </CardContent>
            </Card>
          </main>
...
// Sub-components
// ———————————————————————————————————————————————————————
function SidebarLink({ href, icon, children, onClick }: { href: string; icon: React.ReactNode; children: React.ReactNode; onClick?: () => void }) {
  return (
    <Link href={href} onClick={onClick} className="flex items-center gap-2 rounded-xl px-3 py-2 text-muted-foreground transition-colors hover:bg-muted/40 hover:text-foreground">
      <span className="grid place-items-center rounded-md border p-1.5 text-foreground/80">{icon}</span>
      <span>{children}</span>
    </Link>
  );
}
...
function MessagesTable({ messages }: { messages: typeof RECENT_MESSAGES }) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
  ...
  return (
    <div className="relative h-[40px] w-full overflow-hidden rounded-xl border bg-background">
      <div className="absolute inset-0 flex items-end gap-[3px] p-2">
        {values.map((v, i) => (
          <div key={i} className="w-[6px] rounded-sm bg-primary/70" style={{ height: `${(v / max) * height}px` }} />
        ))}
      </div>
...
function MiniBar({ values }: { values: number[] }) {
  ...
  const height = 28;
  return (
    <div className="relative h-[28px] w-full overflow-hidden rounded-md border bg-muted/30">
      <div className="absolute inset-0 flex items-end gap-[2px] p-1.5">
        {values.map((v, i) => (
          <div key={i} className="w-[5px] rounded-[2px] bg-primary/50" style={{ height: `${(v / max) * height}px` }} />
        ))}
      </div>
    </div>
  );
...
/*
Missing Shadcn components? Install:

npx shadcn@latest add card button badge tabs table avatar progress dropdown-menu input separator checkbox switch scroll-area tooltip sheet label
*/
