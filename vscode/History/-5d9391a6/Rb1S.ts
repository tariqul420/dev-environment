...
export function setQuery(prevQS: string, pairs: Record<string, QSValue>): string {
...
export function withPageReset(pairs: Record<string, QSValue>): Record<string, QSValue> {
  return { ...pairs, pageIndex: "1" };
}
...
export function removeKeysFromQuery({
  ...
  const s = new URLSearchParams(params || "");
  keysToRemove.forEach((k) => s.delete(k));
  const base = typeof window !== "undefined" ? window.location.pathname : "";
  const qs = s.toString();
  return qs ? `${base}?${qs}` : base;
}

export const toYMD = (d?: Date) => (d ? new Date(d).toISOString().slice(0, 10) : undefined);
...
export function setCsvParam(prevQS: string, key: string, values: string[], resetPage = true): string {
  const pairs: Record<string, string | undefined> = {
    [key]: values.length ? values.join(",") : undefined,
  };
  return resetPage ? setQuery(prevQS, withPageReset(pairs)) : setQuery(prevQS, pairs);
}

export function arraysShallowEqual(a: readonly string[], b: readonly string[]): boolean {
