...
import { ArrowRight, BadgeDollarSign, Crown, Gem, HeartHandshake, Mail, Rocket, Shield, Star, Users } from 'lucide-react';
import * as React from 'react';

// ----- Safe motion fallback (prevents runtime crash if motion is undefined) -----
const MotionDiv: React.ComponentType<React.HTMLAttributes<HTMLDivElement>> =
  // @ts-expect-error - runtime guard
  (motion && motion.div) || ((props: any) => <div {...props} />);

// ----- Anim presets -----
const fadeUp = { initial: { opacity: 0, y: 8 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.4 } };
const inView = { hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0, transition: { duration: 0.4 } } };
...
const tiers = [
  ...
  },
  {
    ...
    icon: Gem,
    highlight: false,
    perks: ['Everything in Bronze', 'Logo on homepage carousel', 'Social shoutouts (monthly)', 'Priority support for requests'],
  },
  {
    name: 'Gold',
    price: '$399/mo',
    icon: Crown,
    highlight: true,
    perks: ['Everything in Silver', 'Dedicated feature spotlight', 'Blog post mention', 'Custom discount code'],
  },
  {
    name: 'Platinum',
    price: '$999/mo',
    icon: Rocket,
    highlight: false,
    perks: ['Everything in Gold', 'Co-branded tutorial/tool', 'Quarterly planning call', 'Logo in app footer (opt-in)'],
  },
] as const;

const benefits = [
  { title: '2M+ yearly pageviews', icon: Users, desc: 'Across utilities like PDF, Image, and Dev tools.' },
  { title: 'Search-first audience', icon: BadgeDollarSign, desc: 'High-intent users from SEO & direct traffic.' },
  { title: 'Brand-safe content', icon: Shield, desc: 'Utilities only—no controversial topics.' },
  { title: 'Real usage visibility', icon: HeartHandshake, desc: 'Opt-in anonymized analytics showcases.' },
];

const faqs = [
  { q: 'How are logos displayed?', a: 'Your logo appears here and (for higher tiers) in a rotating carousel on the homepage & tool pages.' },
  { q: 'Can I sponsor a specific tool?', a: 'Yes—mention the tool in your message for targeted placement.' },
  { q: 'Do you accept annual billing?', a: 'Absolutely. Annual plans include a discount and an invoice.' },
  { q: 'What creative assets do you need?', a: 'A horizontal SVG/PNG logo, a one-liner, and an optional UTM link.' },
];
  ...
  return (
    <div className="relative">
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-10 sm:py-14">
        <MotionDiv {...fadeUp}>
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Power useful tools for millions</h1>
            <p className="mt-3 max-w-2xl text-balance text-sm text-muted-foreground sm:text-base">
              Help us keep Tools Hub free and blazing-fast while putting your brand in front of high-intent makers and professionals. Flexible tiers. Brand-safe placement.
            </p>
      ...
      <section className="mx-auto mt-10 w-full max-w-6xl px-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {tiers.map((t) => {
            const Icon = t.icon;
            return (
              <Card key={t.name} className={cn('relative overflow-hidden rounded-2xl border bg-background/60 shadow-sm backdrop-blur transition', t.highlight && 'ring-2 ring-primary/30')}>
                <CardHeader>
                  <div className="flex items-center gap-2">
                    <Icon className={cn('h-5 w-5', t.highlight && 'text-primary')} />
                    <CardTitle>{t.name}</CardTitle>
                  </div>
                  <CardDescription>Great for {t.name === 'Bronze' ? 'getting started' : t.name.toLowerCase()} visibility</CardDescription>
                </CardHeader>
                <CardContent>
      ...
      <section className="mx-auto mt-12 w-full max-w-6xl px-4">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold tracking-tight sm:text-2xl">Brands that believe in us</h2>
          <span className="text-sm text-muted-foreground">(Sample logos)</span>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {logos.map((l) => (
            <div key={l.id} className="group rounded-2xl border bg-background/50 p-4 shadow-sm backdrop-blur">
        ...
        <div className="relative overflow-hidden rounded-3xl border bg-gradient-to-r from-primary/15 via-primary/10 to-transparent p-6 shadow-sm backdrop-blur">
          <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">We’ll craft a placement that feels native, useful, and brand-safe.</p>
            </div>
      ...
      <section className="mx-auto mt-12 w-full max-w-6xl px-4">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {faqs.map((f) => (
            <div key={f.q} className="rounded-2xl border bg-background/60 p-5 shadow-sm backdrop-blur">
      ...
      <section id="contact" className="mx-auto mt-12 w-full max-w-3xl px-4">
        <div className="rounded-3xl border bg-background/60 p-6 shadow-sm backdrop-blur">
          ...
          <form className="grid gap-4 sm:grid-cols-2">
            ...
            <Textarea placeholder="What would you like to achieve?" className="min-h-28 sm:col-span-2" />
      ...
      {/* Media Kit */}
      <section id="media-kit" className="mx-auto my-12 w-full max-w-6xl px-4">
        <div className="rounded-2xl border bg-background/60 p-5 shadow-sm backdrop-blur">
          <h4 className="font-semibold">Media kit</h4>
          <p className="mt-1 text-sm text-muted-foreground">One-pager with audience, placements, and specs. Add your URL here.</p>
          <div className="mt-3">
...
function DownloadIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" {...props}>
      <path d="M12 3v10m0 0l3.5-3.5M12 13L8.5 9.5M5 16v3h14v-3" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
