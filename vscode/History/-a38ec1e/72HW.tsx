...
function uuidV4({
  ...
  // use crypto.randomUUID if available for correctness & speed
...
function ulid({ upper = true }: { upper?: boolean } = {}) {
  // ULID: 48-bit time (ms) + 80-bit randomness, encoded in Crockford Base32 to 26 chars
  const ENCODING = '0123456789ABCDEFGHJKMNPQRSTVWXYZ'; // Crockford (I,L,O,U removed)
  const time = Date.now();
...
function nanoId(len = 12, alphabet = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz') {
...
export default function IdGeneratorClient() {
  ...
  const [nanoAlphabet, setNanoAlphabet] = useState('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz');
    ...
    for (let i = 0; i < Math.max(1, Math.min(1000, count)); i++) {
      switch (mode) {
        ...
        case 'ulid':
          ...
          break;
        case 'nanoid':
          res.push(nanoId(Math.max(4, Math.min(64, nanoLen)), nanoAlphabet || '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'));
          break;
        case 'hex':
    ...
    <>
      <ToolPageHeader
        ...
        actions={
          <>
            ...
            <ExportTextButton variant="default" filename="ids.txt" getText={() => processed.join('\n')} label="Export .txt" disabled={processed.length === 0} />
            <ExportCSVButton filename="ids.csv" getRows={() => processed.map((x, i) => [i + 1, x])} label="Export .csv" disabled={processed.length === 0} />
          </>
        }
      />
      ...
      <GlassCard className="shadow-sm">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          />
          <InputField label="Count" type="number" min={1} max={1000} value={count} onChange={(e) => setCount(Math.max(1, Math.min(1000, Number(e.target.value) || 1)))} />
          <SelectField
          ...
          {/* Global post-processing */}
          <InputField label="Add Prefix" value={prefix} onChange={(e) => setPrefix(e.target.value)} />
          <InputField label="Add Suffix" value={suffix} onChange={(e) => setSuffix(e.target.value)} />
          <div className="grid gap-2">
            <SwitchRow label="Ensure unique (within batch)" checked={ensureUnique} onCheckedChange={setEnsureUnique} />
          ...
          {mode === 'uuid' && (
            <>
              <div className="grid gap-2">
                ...
                <SwitchRow label="Remove hyphens" checked={uuidNoHyphen} onCheckedChange={setUuidNoHyphen} />
              </div>
            </>
          )}
          ...
          {mode === 'nanoid' && (
            <>
              <InputField label="Length" type="number" min={4} max={64} value={nanoLen} onChange={(e) => setNanoLen(Math.max(4, Math.min(64, Number(e.target.value) || 4)))} />
              <InputField label="Alphabet" value={nanoAlphabet} onChange={(e) => setNanoAlphabet(e.target.value)} hint="Leave blank to use default A–Z, a–z, 0–9" />
            </>
          )}

          {mode === 'hex' && (
            <>
              <InputField label="Length (chars)" type="number" min={4} max={128} value={hexLen} onChange={(e) => setHexLen(Math.max(4, Math.min(128, Number(e.target.value) || 16)))} />
            </>
          )}

          {mode === 'order' && (
            <>
              <InputField label="Prefix" value={orderPrefix} onChange={(e) => setOrderPrefix(e.target.value)} />
              <div className="grid gap-2">
                <SwitchRow label="Include YYYYMMDD date" checked={orderIncludeDate} onCheckedChange={setOrderIncludeDate} />
              </div>
            </>
          )}
      ...
      <GlassCard className="shadow-sm">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex gap-2">
            <ActionButton icon={Wand2} label="Copy All" onClick={() => navigator.clipboard.writeText(copyAllText())} disabled={processed.length === 0} variant="outline" />
            <ExportTextButton filename="ids.txt" getText={() => processed.join('\n')} label="Export .txt" disabled={processed.length === 0} />
          </div>

          {processed.length === 0 ? (
            <p className="text-sm text-muted-foreground">No IDs yet. Configure settings and click Generate.</p>
          ) : (
