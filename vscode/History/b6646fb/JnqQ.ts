...
type AdminUserRow = {
  ...
  totalSpent: string; // serialized decimal
...
export async function getUsersForAdmin({
  ...
  try {
    ...
    // Aggregate orders for these users (all statuses)
    ...
    // Aggregate completed orders only (DELIVERED or CONFIRMED)
    ...
    // Address counts for these users
    ...
    // Which users have a default address
    ...
    const ordersMap = new Map(
      ordersAgg.map((r) => [
        r.userId as string,
        {
          ...
          totalSpent: r._sum.total, // Decimal
    ...
    const rows: AdminUserRow[] = users.map((u) => {
      ...
      return {
        ...
        totalSpent: (o?.totalSpent ?? (0 as any)).toString(), // serialize Decimal
    ...
    // Ensure plain JSON (Dates & strings are fine; Decimals serialized above)
    return {
    ...
    throw new Error("Failed to fetch users", { cause: error as any });
  }
}
