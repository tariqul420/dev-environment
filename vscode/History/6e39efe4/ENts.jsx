...
export default async function PaymentDetailsPage({ params }) {
  // ⚠️ params is not a promise—no await
  const { id } = params;
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* NEW: Totals by this phone */}
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border p-3">
              <div className="text-xs text-muted-foreground">Total by This Phone</div>
              <div className="mt-1 font-semibold">
                {fmtAmount(payment.totalAmount)} {cur}
              </div>
            </div>
            <div className="rounded-xl border p-3">
              <div className="text-xs text-muted-foreground">Payments Count (This Phone)</div>
              <div className="mt-1 font-semibold">{payment.paymentCount || 0}</div>
            </div>
          </div>
