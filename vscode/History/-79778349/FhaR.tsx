    ...
    <FormField
      ...
      render={({ field }) => {
        ...
        const defaultSummary = () => {
          ...
          if (!showChips) {
            return <span>{labels.length > 2 ? `${labels.length} selected` : labels.join(', ')}</span>;
          }
          return (
                  ...
                  <Command>
                    ...
                    <div className="flex items-center gap-2 px-2 pt-2">
                      ...
                      <div className="ml-auto flex items-center gap-1">
                        <Button type="button" variant="ghost" size="sm" onClick={allSelected ? clearAll : selectAll} disabled={!options.length}>
                      ...
                      <ScrollArea style={{ maxHeight: listMaxHeight }}>
                        <CommandGroup>
                          {options.map((opt) => {
                            ...
                            return (
                              <CommandItem key={opt.value} value={`${opt.label} ${opt.value}`} className="cursor-pointer" onSelect={() => toggle(opt.value)}>
