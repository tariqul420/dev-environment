      ...
      <GlassCard>
        <CardContent className="flex items-center justify-between ">
          ...
          <SelectField
            ...
            options={PRESETS_SIZE.map(([w, h, label]) => ({
              value: `${w}x${h}`,
              label,
            }))}
            value={`${s.w}x${s.h}`}
                ...
                <div className="space-y-2">
                  <InputField
                    ...
                    onFilesChange={(files) => {
                      const f = files?.[0];
                      setS((p) => {
                        if (p.bgImage && p.bgImage.startsWith("blob:"))
                          URL.revokeObjectURL(p.bgImage);
                ...
                <div className="space-y-2">
                  <InputField
                    ...
                    onFilesChange={(files) => {
                      const f = files?.[0];
                      setS((p) => {
                        if (p.logo && p.logo.startsWith("blob:")) URL.revokeObjectURL(p.logo);
