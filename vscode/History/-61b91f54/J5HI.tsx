...
function hasAlpha(img: CanvasImageSource, w: number, h: number): boolean {
  const c = document.createElement("canvas");
  c.width = w;
  c.height = h;
  ...
  try {
    // May throw if the canvas is tainted by a cross-origin image
    data = ctx.getImageData(0, 0, w, h).data;
  } catch {
    // Decide your policy here. Fallback to false = assume no alpha.
    return false;
  }
