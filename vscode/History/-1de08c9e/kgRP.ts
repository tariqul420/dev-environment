...
const COMPLETED_STATUSES = ["delivered", "confirmed"] as const;
...
export async function getSellingReport(monthsBack = 3): Promise<ChartData[]> {
  try {
    await dbConnect();
    ...
    const pipeline: PipelineStage[] = [
      // Use Date objects for accurate range filtering
      { $match: { createdAt: { $gte: start, $lte: end } } },

      // Normalize a YYYY-MM-DD string for grouping/label
      {
        $addFields: {
          dateOnly: {
      ...
      // Join product to get price/salePrice; keep null-safe
      {
        $lookup: {
          from: "products",
          localField: "product",
      ...
      // Compute per-document helpers
      {
        $addFields: {
          _isCompleted: { $in: ["$status", COMPLETED_STATUSES] },
      ...
      // Group per day
      {
        $group: {
          ...
          // all orders count (no status filter)
          orderCount: { $sum: 1 },

          // completed orders count (delivered/confirmed)
          completed: {
            $sum: { $cond: ["$_isCompleted", 1, 0] },
          },

          // items sold from completed orders only
          salesCount: {
            $sum: { $cond: ["$_isCompleted", "$_qty", 0] },
          },
    ...
    // If you have a TypeScript aggregate model type, you can pass it here
    ...
    logger.error("Error getting selling report:", err);
    throw err;
  }
}
