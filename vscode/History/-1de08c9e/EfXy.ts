...
export async function getSellingReport() {
  try {
    ...
    const pipeline = [
      ...
      // Need product price for revenue
      {
        $lookup: {
          from: "products",
          localField: "product",
      ...
      // Group per day
      {
        $group: {
          ...
          // items sold from delivered/confirmed
          salesCount: {
            $sum: {
              $cond: [
          ...
          // revenue from delivered/confirmed
          totalRevenue: {
            $sum: {
              $cond: [
                { $in: ["$status", ["delivered", "confirmed"]] },
    ...
    return daily.map((d) => ({
      date: d._id,
      sales: d.salesCount, // items from delivered/confirmed
      revenue: d.totalRevenue, // revenue from delivered/confirmed
      orders: d.orderCount, // all orders
      completed: d.completed, // delivered/confirmed orders count
    }));
  } catch (error) {
