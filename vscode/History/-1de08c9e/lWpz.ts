...
export async function getSellingReport() {
  try {
    ...
    // Get the current date and calculate the start of the last 3 months
    const currentDate = new Date();
    const threeMonthsAgo = new Date(currentDate);
    threeMonthsAgo.setMonth(currentDate.getMonth() - 3);

    // Pipeline for salesCount (no status filter)
    const salesPipeline = [
      {
        $addFields: {
          dateOnly: {
    ...
    // Pipeline for revenue and orderCount (with status filter)
    const revenueAndOrdersPipeline = [
      {
        $match: {
          status: { $in: ["delivered", "confirmed"] },
        },
    ...
    const [salesData, revenueAndOrdersData] = await Promise.all([
      Order.aggregate(salesPipeline),
      Order.aggregate(revenueAndOrdersPipeline),
    ]);

    // Merge results
    const chartData = salesData.map((salesItem) => {
      const revenueAndOrdersItem = revenueAndOrdersData.find(
        (item) => item._id === salesItem._id,
      );
      return {
        date: salesItem._id,
        sales: salesItem.salesCount, // From salesPipeline (no status filter)
        revenue: revenueAndOrdersItem?.totalRevenue || 0, // From revenueAndOrdersPipeline (with status filter)
        orders: revenueAndOrdersItem?.orderCount || 0, // From revenueAndOrdersPipeline (with status filter)
      };
    });

    // Include dates from revenueAndOrdersData that might not be in salesData
    const additionalDates = revenueAndOrdersData
      .filter((item) => !salesData.some((sales) => sales._id === item._id))
      .map((item) => ({
        date: item._id,
        sales: 0, // No sales for this date in salesData
        revenue: item.totalRevenue,
        orders: item.orderCount,
      }));

    // Combine and sort by date
    const finalChartData = [...chartData, ...additionalDates].sort(
