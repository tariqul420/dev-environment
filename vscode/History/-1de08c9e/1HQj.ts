...
export async function getSellingReport() {
  try {
    ...
    // Date window: last 3 months (by createdAt day)
    const currentDate = new Date();
    ...
    // One-shot aggregation with $facet
    const [result] = await Order.aggregate([
      // derive dateOnly once so all facets can reuse
      {
        $addFields: {
          dateOnly: {
      ...
      // Restrict to last 3 months window
      {
        $match: {
      ...
      {
        $facet: {
          // 1) All orders per day (any status)
          allOrders: [
            {
              $group: {
                _id: "$dateOnly",
          ...
          // 2) Completed+Revenue facet (confirmed OR delivered)
          completedAndRevenue: [
            {
            ...
            },
            {
              $group: {
                _id: "$dateOnly",
                // confirmed OR delivered count
                completed: { $sum: 1 },
                // delivered orders count only (for "Sales")
                sales: {
                  $sum: {
                    $cond: [{ $eq: ["$status", "delivered"] }, 1, 0],
                  },
                },
                // revenue from confirmed OR delivered
                revenue: {
                  $sum: {
                    $multiply: [
                      { $ifNull: ["$quantity", 1] },
    ...
    const allOrders = (result?.allOrders ?? []) as Array<{
      _id: string;
      orders: number;
    }>;
    const completedAndRevenue = (result?.completedAndRevenue ?? []) as Array<{
      _id: string;
      completed: number;
      sales: number;
      revenue: number;
    ...
    const byDate = new Map<
      string,
      { order: number; completed: number; sales: number; revenue: number }
    >();
    ...
    for (const r of completedAndRevenue) {
      const existing = byDate.get(r._id) ?? {
        order: 0,
        completed: 0,
        sales: 0,
        revenue: 0,
    ...
    // Return as sorted array for chart
    const chartData = Array.from(byDate.entries())
      .sort((a, b) => (a[0] < b[0] ? -1 : 1))
      .map(([date, v]) => ({
        date,
        order: v.order, // ✅ All orders
        completed: v.completed, // ✅ Confirmed + Delivered
        sales: v.sales, // ✅ Delivered orders count
        revenue: v.revenue, // ✅ Sum from confirmed+delivered
      }));
