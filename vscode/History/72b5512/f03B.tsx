...
type Option = {
  ...
  value: string | number; // allow numbers in options; normalized to string for Select
  disabled?: boolean;
};
...
const CLEAR_TOKEN = '__CLEAR__'; // used internally to avoid empty-string SelectItem
...
function normalizeOut(raw: string, { valueAsNumber }: { valueAsNumber?: boolean }): string | number | undefined {
  ...
  // Convert to number if possible; otherwise undefined when clearing
  const n = Number(raw);
  ...
  if (!name) {
    ...
    return (
      <div className={className}>
        {label ? (
          <label htmlFor={triggerId} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
            {label}
  ...
  return (
    <FormField
      ...
      render={({ field }) => {
        ...
        // externalValue takes precedence; fall back to RHF value; then ''
