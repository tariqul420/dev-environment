"use client";

import { Button } from "@/components/ui/button";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover";
import clsx from "clsx";
import {
  Facebook,
  HelpCircle,
  Mail,
  MessageCircle,
  Phone,
  X,
} from "lucide-react";
import { useState } from "react";

export default function HelpWidget() {
  const [open, setOpen] = useState(false);

  return (
    <div className="fixed right-6 bottom-6 z-50">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="secondary"
            className={clsx(
              "relative rounded-full p-4 shadow-xl transition-all duration-300",
              open
                ? "bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-white"
                : "bg-white",
            )}
          >
            <span className="absolute inset-0 animate-pulse rounded-full bg-gradient-to-tr from-purple-400 to-pink-400 opacity-30 blur-sm"></span>
            <span className="relative z-10">
              {open ? (
                <X className="h-5 w-5" />
              ) : (
                <HelpCircle className="h-5 w-5" />
              )}
            </span>
          </Button>
        </PopoverTrigger>

        <PopoverContent className="w-72 rounded-xl p-4 shadow-2xl">
          <div className="mb-4 text-center">
            <p className="text-lg font-semibold">
              👋 কিভাবে সাহায্য করতে পারি?
            </p>
          </div>

          <div className="space-y-3">
            {/* WhatsApp */}
            <a
              href="https://wa.me/880123456789"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:bg-muted flex items-center gap-3 rounded-md p-2 transition"
            >
              <MessageCircle className="h-5 w-5 text-green-500" />
              <span>WhatsApp করুন</span>
            </a>

            {/* Messenger */}
            <a
              href="https://m.me/naturalsefa"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:bg-muted flex items-center gap-3 rounded-md p-2 transition"
            >
              <Facebook className="h-5 w-5 text-blue-600" />
              <span>Messenger</span>
            </a>

            {/* Call */}
            <a
              href="tel:+880123456789"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:bg-muted flex items-center gap-3 rounded-md p-2 transition"
            >
              <Phone className="h-5 w-5 text-emerald-600" />
              <span>কল করুন</span>
            </a>

            {/* Email */}
            <a
              href="mailto:support@naturalsefa.com"
              className="hover:bg-muted flex items-center gap-3 rounded-md p-2 transition"
            >
              <Mail className="h-5 w-5 text-indigo-600" />
              <span>ইমেইল পাঠান</span>
            </a>

            {/* Contact Page */}
            <a
              href="/contact-us"
              className="hover:bg-muted flex items-center gap-3 rounded-md p-2 transition"
            >
              <HelpCircle className="h-5 w-5 text-purple-600" />
              <span>Contact Page</span>
            </a>
          </div>
        </PopoverContent>
      </Popover>
    </div>
  );
}
