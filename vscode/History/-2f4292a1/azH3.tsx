...
interface DataTableProps<T extends BaseRecord> {
  ...
  columns?: ColumnDef<T>[];
  pageSize?: number;
  pageIndex?: number; // 1-based from server
  total?: number;
  uniqueIdProperty?: string; // defaults to "id"
...
function getRowIdValue<T extends BaseRecord>(row: T, uniqueIdKey: string) {
  const raw = (row as any)[uniqueIdKey] ?? row.id ?? row._id;
...
export default function DataTable<T extends BaseRecord>({
  ...
  const [pagination, setPagination] = React.useState<PaginationState>({
    pageIndex: Math.max(0, pageIndex - 1), // react-table is 0-based
    pageSize,
  });
