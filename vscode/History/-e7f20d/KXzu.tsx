...
export default function CurrencyConverterPage() {
  ...
  async function fetchRates(baseCode: string) {
    ...
    try {
      ...
      if (cachedRaw) {
        const cached = JSON.parse(cachedRaw) as { updatedAt: string; rates: RatesMap };
      ...
      const res = await fetch(`https://api.exchangerate.host/latest?base=${encodeURIComponent(baseCode)}`);
      if (!res.ok) throw new Error('Failed to fetch');
      const data = await res.json();
      if (!data?.rates) throw new Error('Invalid response');

      const updatedAt = nowISO();
      localStorage.setItem(cacheKey, JSON.stringify({ updatedAt, rates: data.rates as RatesMap }));

      setRates(data.rates as RatesMap);
      setLastUpdated(updatedAt);
      setStatus('ok');
    } catch (e) {
      // As a graceful fallback, try any cached version even if stale
      const cacheKey = `cc_rates_${baseCode}`;
      const cachedRaw = localStorage.getItem(cacheKey);
      if (cachedRaw) {
        const cached = JSON.parse(cachedRaw) as { updatedAt: string; rates: RatesMap };
  ...
  function exportHistoryCSV() {
    const rows = [['Time', 'From', 'To', 'Amount', 'Rate', 'Result'], ...history.map((h) => [h.ts, h.from, h.to, h.amount, h.rate, h.result])];
