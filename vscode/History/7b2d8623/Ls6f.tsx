...
export async function generateMetadata({ params }: SlugParams): Promise<Metadata> {
  const { slug } = params;
  const project = (await getProjectBySlug(slug)) as IProject | null;
  ...
  // OG image priority: coverImage → first screenshot → default OG
  ...
  const modifiedTime = (project as any).updatedAt ? new Date((project as any).updatedAt).toISOString() : publishedTime;
