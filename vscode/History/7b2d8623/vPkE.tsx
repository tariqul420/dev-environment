...
export async function generateMetadata({ params }: SlugParams) {
  ...
  const title = project.title ? `${project.title} | Portfolio Project` : 'Portfolio Project';

  const description = project.shortDescription || project.description?.replace(/<[^>]+>/g, '').slice(0, 160) || 'A project from Tariqul Islam portfolio.';

  const url = `https://YOUR_DOMAIN/projects/${project.slug}`; // Replace with your real domain
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <p className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground capitalize">{project.category ?? 'N/A'}</span>
                  </p>
                  <p className="flex items-center gap-2">
