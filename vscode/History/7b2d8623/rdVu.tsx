...
import { getBlogBySlug } from '@/lib/actions/blog.action'; // ← adjust path if different
...
export async function generateMetadata({ params }: SlugParams): Promise<Metadata> {
  ...
  if (!blog) {
    return {
      ...
      title: 'Post Not Found | Tariqul Islam',
  ...
  const title = (blog.seo?.title || blog.title) + ' | Tariqul Islam';
  const rawDesc = blog.seo?.description || blog.description || stripHtml(blog.content || '') || 'An article from Tariqul Islam’s blog.';
  ...
  // Optional fields if available in your model:
  ...
  const authorName = (typeof blog.author === 'object' && blog.author?.name) || 'Tariqul Islam';
  ...
  return {
    ...
    keywords: ['Tariqul Islam blog', 'Web development', 'Next.js', 'React', 'TypeScript', 'Full-stack', ...(blog.tags || [])],
    category: 'Blog',
  };
}
