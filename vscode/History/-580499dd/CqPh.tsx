...
import { Check, ClipboardCopy as Copy, Eye, EyeOff, Info, Key, RotateCcw, Shield, ShieldAlert, ShieldCheck, Timer } from 'lucide-react';
      ...
      <GlassCard className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-6">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-semibold tracking-tight">
            <ShieldCheck className="h-6 w-6" /> Password Strength
          </h1>
          <p className="text-sm text-muted-foreground">Check password entropy, estimated crack times, and get actionable hints to harden your password.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={resetAll} className="gap-2">
            <RotateCcw className="h-4 w-4" /> Reset
          </Button>
          <Button variant="outline" onClick={copyNow} className="gap-2" disabled={!pw}>
            {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />} Copy
          </Button>
        </div>
      </GlassCard>

      <ToolPageHeader icon={ShieldCheck} title="Password Strength" description="Check password entropy, estimated crack times, and get actionable hints to harden your password." />
