...
export default function DashboardHeader({ role }: { role: 'admin' | 'customer' }) {
          ...
          {role === 'admin' && (
            <Badge
              ...
              <span className="relative flex h-2.5 w-2.5 items-center justify-center">
                {pulse && <span className={cn('absolute inline-flex h-full w-full animate-ping rounded-full opacity-70', isOnline ? 'bg-emerald-400' : 'bg-rose-400')} />}
                <span className={cn('relative inline-flex h-2.5 w-2.5 rounded-full', isOnline ? 'bg-emerald-400' : 'bg-rose-400')} />
              </span>
