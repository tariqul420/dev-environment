...
import { ActionButton, ExportCSVButton, ResetButton } from "@/components/shared/action-buttons"; // if you have this helper
...
type InvoiceData = {
  ...
  from: Party;
  to: Party;
  currency: string; // ISO 4217 code or symbol (BDT/USD/€)
...
function fmtCurrency(amount: number, currency: string, mode: "code" | "symbol") {
  ...
  // symbol mode – prepend symbol (e.g. ৳, $, €)
  return `${currency}${value.toFixed(2)}`;
}
      ...
      <ToolPageHeader
        ...
        actions={
          <>
            ...
            <ActionButton icon={Save} label="Save JSON" onClick={exportJSON} />
            <ActionButton
              icon={FileCheck2}
              label="Load JSON"
        ...
        <GlassCard className="shadow-sm">
          ...
          <CardContent className="space-y-3">
            ...
            {data.items.map((it) => {
              ...
              return (
                <div key={it.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 rounded-lg p-3">
                  <div className="md:col-span-4 space-y-2">
                    <label className="md:hidden text-xs text-muted-foreground">Name</label>
                    <InputField
                      value={it.name}
                  ...
                  <div className="md:col-span-1 flex md:items-start md:justify-end gap-2">
                    <ActionButton size="icon" aria-label="Clone" onClick={() => cloneRow(it.id)}>
                ...
                <div className="flex justify-between font-semibold text-base pt-1">
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    <ActionButton
                      size="sm"
                      variant="outline"
                      icon={copied === "grand" ? Check : FileText}
                      label="Copy"
                      onClick={copyTotal}
                    />
                  </span>
