...
import {
  ...
  CopyButton,
  ExportTextButton,
  PasteButton, // ✅ keep this import; we’re using your shared PasteButton API
  ResetButton,
        ...
        <GlassCard>
          <div className="p-4">
            <div className="mb-2 flex items-center justify-between">
              ...
              {/* ✅ Use your shared PasteButton properly */}
              <PasteButton
                mode="replace"
                smartNewline={false}
                label="Paste"
                pastedLabel="Pasted"
                // Keep existing value (for append modes, but here we replace)
                getExisting={() => token}
                // When paste completes, update token & decode
                setValue={(next) => {
                  const cand = extractToken(next) || next;
                  setToken(cand);
                  decodeToken(cand);
                }}
                // Also fire on raw text (handy if you ever flip to append mode)
                onText={(text) => {
                  if (!autoOnPaste) return;
