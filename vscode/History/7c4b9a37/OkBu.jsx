...
export default function AdminPaymentTableMenu({ row }) {
  ...
  async function HandleDeleteProject() {
    try {
      toast.promise(
        deletePayment({
          paymentId: row.original.id || row.original._id,
          path: pathName,
        }),
        {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-32">
        <DropdownMenuItem>
          <Link
            href={`/admin/payments/details/${row.original.payment.id || row.original.payment._id}`}
        ...
        <DropdownMenuItem>
          <Link
            href={`/admin/payments/${row.original.id || row.original.payment._id}`}
            className="block w-full"
          >
            Edit
          </Link>
