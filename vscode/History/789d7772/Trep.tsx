...
export default function AdminUserTableMenu({ row }: { row: Row<UserRecord> }) {
  ...
  const handleChangeRole = async (userId: string, selectedRole: UserRole) => {
    try {
      toast.promise(updateUserRole({ userId, role: selectedRole }), {
        loading: `Changing role to ${selectedRole}...`,
        success: `Role updated to ${selectedRole} successfully!`,
