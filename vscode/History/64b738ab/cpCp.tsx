...
import { Download, Info, RefreshCw, Settings2, Table } from "lucide-react";
...
export default function CsvJsonClient() {
    ...
    const toRows = (): string[][] => {
      if (Array.isArray(data)) {
        ...
        if (Array.isArray(data[0])) {
          // array of arrays
          return data.map((r: unknown[]) => r.map((v) => String(v ?? "")));
      ...
      if (data && typeof data === "object") {
        ...
        const row = keys.map((k) => String(obj[k] ?? ""));
  ...
  // ✅ Minimal & correct dependencies: rerun when autoRun flag or convert function identity changes
  React.useEffect(() => {
    if (autoRun) convert();
  }, [autoRun, convert]);
    ...
    <>
      ...
      <GlassCard>
        <CardHeader className="flex items-center justify-between px-3">
          <CardTitle className="text-sm font-medium opacity-80">Result</CardTitle>
