...
type BaseProps = Omit<
  ...
  "onChange" | "type" | "value"
> & {
  /** Optional leading icon shown in the label (not the file button). */
  icon?: LucideIcon;

  /** Label content; if both provided, labelNode wins. */
  ...
  /** Supports boolean for checkbox in standalone mode. */
  value?: string | number | boolean;
  defaultValue?: string | number | boolean;

  /** Show a small red * mark after label */
  requiredMark?: boolean;

  /** Hint text below the control */
  hint?: string;

  /** Standard input onChange for text/number/file. */
  onChange?: React.ChangeEventHandler<HTMLInputElement>;

  /** HTML type, supports "checkbox" as well. */
  type?: HTMLInputTypeAttribute;

  /** Parse number when type is number (or when true). */
  parseNumber?: boolean;
...
> = BaseProps & {
  /** If provided, RHF mode; if omitted, standalone mode. */
  name?: TName;
};
...
function toTextInputValue(v: unknown): string | number {
  ...
  if (typeof v === "boolean") return v ? "1" : ""; // or String(v) if you prefer "true"/"false"
