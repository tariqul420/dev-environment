...
type BaseProps = Omit<React.InputHTMLAttributes<HTMLInputElement>, 'onChange' | 'type' | 'value'> & {
  /** RHF field name (optional). If omitted, the component runs in standalone mode. */
  name?: string;
  ...
  /** Convert number inputs to `number` in onChange */
  parseNumber?: boolean;
  /** Blur number inputs on wheel to avoid accidental changes */
  ...
  /** File UI options (no popover, only icon + text + optional Clear) */
  fileIcon?: React.ReactNode; // default: <Upload ... />
  fileButtonLabel?: string; // default: 'Import'
  fileButtonVariant?: ButtonVariant; // default: 'outline'
  fileButtonSize?: ButtonSize; // default: 'default'
};
              ...
              <FormControl>
                {isFile ? (
                  <div className="flex items-center gap-2">
                    ...
                    {hasSelection && (
                      <Button type="button" variant="ghost" size="sm" onClick={clearFilesRHF} disabled={effectiveDisabled || field.disabled}>
                        {fileClearLabel}
                      </Button>
                    )}
                  </div>
    ...
    <div className={className}>
      ...
      {isFile ? (
        <div className="mt-1 flex items-center gap-2">
          ...
          {hasSelection && (
            <Button type="button" variant="ghost" size="sm" onClick={clearFilesStandalone} disabled={effectiveDisabled}>
              {fileClearLabel}
            </Button>
          )}
        </div>
