...
> & {
  ...
  defaultValue?: string | number | boolean; // ✅ allow boolean for checkbox
  requiredMark?: boolean;
  hint?: string;
  onChange?: React.ChangeEventHandler<HTMLInputElement>;
  type?: HTMLInputTypeAttribute; // includes "checkbox"
  parseNumber?: boolean;
...
>({
  ...
  const shouldParseNumber = !isCheckbox && (parseNumber ?? type === "number"); // ✅ never for checkbox
  ...
  const [internal, setInternal] = React.useState<string | number | boolean | undefined>(defaultValue);
        ...
        render={({ field }) => {
          ...
          return (
            <FormItem className={className}>
              {/* For checkbox, we’ll render label next to control; for others, keep top label */}
                ...
                {isFile ? (
                  ...
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      // onCheckedChange -> boolean | "indeterminate"
                      onCheckedChange={(v) => rhfOnChange(Boolean(v))}
                      disabled={effectiveDisabled || field.disabled}
                      {...restField}
                      // do not spread {...rest} into shadcn Checkbox; it’s a different API
                    />
                    {labelContent ? (
  ...
  return (
    <div className={className}>
      {/* For checkbox, render label to the right; otherwise top label */}
      {!isCheckbox && labelContent ? (
