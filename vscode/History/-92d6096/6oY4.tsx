...
export default function Navbar() {
  ...
  const navLinks: NavLink[] = [
    ...
    isSignedIn
      ? { href: `/${user?.publicMetadata.role}`, label: "Dashboard" }
      : false,
  ];

  // Filter out false values and create a typed array
  const filteredNavLinks = navLinks.filter(
    (link): link is { href: string; label: string } => link !== false,
  );

  return (
    <nav
      className={cn(
        "dark:bg-dark-lite font-grotesk bg-light fixed top-0 z-[20] w-full bg-gradient-to-r p-4 shadow-sm backdrop-blur transition-transform duration-300",
    ...
    >
      <div className="mx-auto flex w-[90vw] max-w-7xl items-center justify-between md:ps-5">
            ...
            {filteredNavLinks.map((link, index) => (
              <li key={index}>
                <Link href={link.href} className={"group relative px-1"}>
                  {link.label}
                  {isActive(link.href) && (
                    <span className="dark:bg-light absolute -bottom-0.5 left-0 h-[1.5px] w-full rounded-full bg-black"></span>
                  )}
                  {!isActive(link.href) && (
                    <span className="dark:bg-light absolute -bottom-0.5 left-0 h-[1.5px] w-0 origin-left rounded-full bg-black duration-300 group-hover:w-full"></span>
                  )}
                </Link>
              </li>
            ))}
        ...
        <div className="flex items-center space-x-4">
          ...
          <Sheet>
            <SheetTrigger
              className="dark:text-light cursor-pointer rounded px-1 py-1 lg:hidden"
                ...
                {filteredNavLinks.map((link) => (
                  <li key={link.href}>
                    <SheetClose asChild>
                      <Link
                        href={link.href}
                        className={"group dark:text-light-bg relative px-1"}
                      >
                        {link.label}
                        {isActive(link.href) && (
                          <span className="dark:bg-light absolute -bottom-0.5 left-0 h-[1.5px] w-full rounded-full bg-black"></span>
