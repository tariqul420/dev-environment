...
for (const m of matches) {
  ...
  const hash = btoa(encodeURIComponent(JSON.stringify(payload)));
  const copyURL = `${window.location.origin}/tools/dev/regex-tester#${hash}`;
      ...
      <ToolPageHeader
        ...
        actions={
          <>
            ...
            />
            <CopyButton />
            <ActionButton onClick={exportMatchesJSON} label="JSON" icon={Download} />
            <ExportCSVButton filename="regex-matches.csv" getRows={() => CSVRows} />
          </>
        }
      />
