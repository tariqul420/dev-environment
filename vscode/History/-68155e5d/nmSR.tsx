...
${html}
  ...
  const handleImport = async (file?: File) => {
    ...
    setPatch({ md: text, filename: file.name.endsWith(".md") ? file.name : `${file.name}.md` });
  };

  // MDX bridge
  type MDXProvidedComponents = {
    code?: React.ComponentType<{ code: string; language?: string }>;
    [key: string]: any; // other optional MDX-styled elements
  };
      ...
      <ToolPageHeader
        ...
        actions={
          <>
            ...
            />
            <CopyButton label="Copy MD" getText={() => md} />
            <CopyButton label="Copy HTML" getText={async () => await generateHTML()} />
...
function buildMarkdownComponents(mdx: MDXProvided): Components {
  ...
  return {
    ...
    table: (props) =>
      m.table ? (m.table as any)(props) : <table className="my-4 w-full table-auto" {...props} />,
    thead: (props) =>
