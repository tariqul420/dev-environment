    ...
    <>
      ...
      <GlassCard>
        <CardContent>
          ...
          {/* Status bar */}
          <div className="flex flex-wrap items-center justify-between rounded-lg border bg-muted/30 px-3 py-2 text-xs mt-3">
...
function buildMarkdownComponents(mdx: MDXProvidedComponents): Components {
  ...
  return {
    /* ---------- FIXED: map ReactMarkdown -> CodeBlockProps ---------- */
    code({ inline, className, children, ...rest }: CodeRendererProps) {
      ...
      // Flatten children to plain text for the code prop
      const codeText =
        typeof children === "string" ? children : String(React.Children.toArray(children).join(""));

      if (m.code) {
        // mdx.code is your CodeBlock component which expects { code, language }
        const MDXCodeBlock = m.code as unknown as (p: {
          code: string;
          language?: string;
        }) => JSX.Element;
    ...
    /* headings with anchors (unchanged) */
    h1: (props) =>
    ...
    /* text & lists — prefer your MDX styles when present */
    p: (props) =>
      m.p ? (
        (m.p as any)(props)
      ) : (
