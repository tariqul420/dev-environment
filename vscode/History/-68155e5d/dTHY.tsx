...
/* ---------------- ReactMarkdown x MDX bridge (error-free) ---------------- */
...
function buildMarkdownComponents(mdx: MDXProvidedComponents): Components {
  ...
  return {
    code({ inline, className, children, ...rest }: CodeRendererProps) {
      ...
      if (m.code) {
        // mdx.code expects { code, language }
        type MDXCodeBlockProps = { code: string; language?: string };
        const MDXCodeBlock = m.code as unknown as React.ComponentType<MDXCodeBlockProps>;
    ...
    // Text & lists (prefer MDX styles if provided)
    p: (props) =>
      m.p ? (
        (m.p as any)(props)
      ) : (
