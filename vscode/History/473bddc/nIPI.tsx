      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-6">
        ...
        <div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-full justify-between", !statuses.length && "text-muted-foreground")} aria-label="Filter by status">
                <span>{statuses.length ? `${statuses.length} selected` : "All statuses"}</span>
                <span className="text-xs opacity-70">Filter</span>
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-60 p-2" align="start">
              <div className="mb-2 flex items-center justify-between">
                <span className="text-xs font-medium text-muted-foreground">Select statuses</span>
                <Button variant="ghost" size="sm" onClick={() => applyStatuses([])}>Clear</Button>
              </div>
              <div className="max-h-60 space-y-1 overflow-auto pr-1">
                {ALL_STATUSES.map((s) => {
                  ...
                  return (
                    <label
                      key={s}
                      className={cn("flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 hover:bg-muted/70", checked && "bg-muted")}
                      onClick={() => toggleStatus(s)}
                    >
                      <input type="checkbox" readOnly checked={checked} className="accent-current" />
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" variant="outline" onClick={() => { const next = sortDir === "asc" ? "desc" : "asc"; setSortDir(next); writeParams({ sortDir: next }); }} aria-label="Toggle sort direction">
