...
export default function OGPreviewPage() {
  ...
  // keep selected image index valid as new data arrives
  React.useEffect(() => {
    if (!data) return;
  ...
  React.useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      // only fetch if different from currently loaded URL (if any)
      const current = data?.url ?? "";
      if (hostnameOf(current + "/") !== hostnameOf(url + "/") || data === null) {
        void runFetch(url);
      }
    }, 600);
    return () => window.clearTimeout(t);
  }, [autoFetch, url, runFetch]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <MotionGlassCard>
      ...
      <GlassCard className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-6">
        ...
        <div className="flex flex-wrap gap-2">
          <ActionButton icon={RotateCcw} label="Reset" variant="outline" onClick={resetAll} />
          <ActionButton icon={LinkIcon} label="Paste URL" variant="outline" onClick={pasteUrl} />
          <ActionButton
            icon={loading ? Loader2 : RefreshCcw}
            label={loading ? "Fetching…" : "Fetch"}
            onClick={() => runFetch()}
        ...
        <CardContent className="grid gap-4">
          <div className="flex flex-col gap-2 md:flex-row md:items-end">
            ...
            <div className="grid gap-2 sm:grid-cols-2 md:w-[340px]">
              ...
              <div className="rounded-md border px-3 py-2">
                <div className="flex items-center justify-between">
                  <Label htmlFor="autofetch" className="text-xs text-muted-foreground">
                    Auto fetch
                  </Label>
                  <Switch id="autofetch" checked={autoFetch} onCheckedChange={setAutoFetch} />
                </div>
              </div>
            </div>
          </div>
            ...
            <CardContent className="grid gap-6 lg:grid-cols-2">
              <div className="space-y-3">
                <div className="flex flex-wrap gap-2">
                  <ActionButton
                    icon={Copy}
                    label={copied === "tags" ? "Copied" : "Copy Meta Tags"}
                    variant="outline"
                    size="sm"
                    onClick={copyTags}
                  />
                  ...
                  <div className="mt-2">
                    {showRaw ? (
                      <TextareaField
                        ...
                        textareaClassName="min-h-[220px] font-mono text-xs"
                      />
                    ) : (
